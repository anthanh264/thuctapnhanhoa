...
- Kiến trúc Nginx: 
- ![images](./images/infographic-Inside-NGINX_process-model.png)
NGINX sử dụng một mô hình tiến trình có thể dự đoán và được điều chỉnh phù hợp với các tài nguyên phần cứng hiện có:
* Tiến trình master thực hiện các tác vụ đặc quyền như đọc cấu hình và liên kết với các cổng (binding to ports), sau đó tạo ra một số lượng nhỏ các tiến trình con (ba loại tiếp theo).
* Tiến trình cache loader chạy khi khởi động để tải bộ nhớ đệm dựa trên đĩa vào bộ nhớ, và sau đó thoát. Nó được lên lịch một cách thận trọng, do đó nhu cầu tài nguyên của nó thấp.
* Tiến trình cache manager chạy định kỳ và loại bỏ các mục (prunes entries) khỏi bộ nhớ đệm trên đĩa để duy trì chúng trong phạm vi kích thước đã được cấu hình.
* Các tiến trình worker thực hiện toàn bộ công việc! Chúng xử lý các kết nối mạng, đọc và ghi nội dung vào đĩa, và giao tiếp với các máy chủ upstream
