   ...
   * [10. Labs](#10-labs)
      ...
      + [10.3 Zabbix High Availability  ](#103-zabbix-high-availability)
         ...
         - [10.3.2 Triển khai ](#1032-trin-khai)
            ...
            * [10.3.2.3 Cài đặt và cấu hình Zabbix Web Frontend (trên 192.168.50.200)](#10323-cài-t-và-cu-hình-zabbix-web-frontend-trên-19216850200)
...
<!-- TOC --><a name="12-tính-nng-ni-bt-ca-zabbix"></a>
...
<!-- TOC --><a name="13-các-nh-ngha-thut-ng-liên-quan"></a>
...
<!-- TOC --><a name="2-kin-trúc-zabbix"></a>
## 2. Kiến trúc Zabbix
...
<!-- TOC --><a name="21-zabbix-server"></a>
### 2.1 Zabbix Server 
<!-- TOC --><a name="211-tng-quan"></a>
#### 2.1.1 Tổng quan
...
<!-- TOC --><a name="212-high-availability"></a>
...
<!-- TOC --><a name="22-zabbix-agent"></a>
### 2.2 Zabbix Agent 
<!-- TOC --><a name="221-zabbix-agent"></a>
#### 2.2.1 Zabbix Agent
...
<!-- TOC --><a name="222-zabbix-agent-2"></a>
...
<!-- TOC --><a name="25-zabbix-sender-zabbix-get-zabbix-js"></a>
### 2.5 Zabbix Sender, Zabbix Get & Zabbix Js 
<!-- TOC --><a name="251-zabbix-sender"></a>
#### 2.5.1 Zabbix Sender
...
<!-- TOC --><a name="31-host-and-host-groups"></a>
### 3.1 Host and Host groups 
<!-- TOC --><a name="311-host"></a>
#### 3.1.1 Host
...
<!-- TOC --><a name="322-các-loi-items"></a>
...
<!-- TOC --><a name="323-history-trends"></a>
...
<!-- TOC --><a name="324-ánh-x-giá-tr-value-mapping"></a>
...
<!-- TOC --><a name="332-trigger-expression"></a>
...
<!-- TOC --><a name="341-to-s-kin-trigger"></a>
...
<!-- TOC --><a name="342-các-ngun-s-kin-khác"></a>
...
<!-- TOC --><a name="343-manual-closing-of-problems"></a>
...
<!-- TOC --><a name="36-visualization"></a>
### 3.6 Visualization
<!-- TOC --><a name="361-biu-graphs"></a>
...
<!-- TOC --><a name="3612-biu-tùy-chnh"></a>
...
<!-- TOC --><a name="3612-biu-tùy-bin-nhanh-ad-hoc-graphs"></a>
...
<!-- TOC --><a name="3613-aggregation-functions"></a>
...
<!-- TOC --><a name="362-s-mng-network-maps"></a>
...
<!-- TOC --><a name="363-bng-iu-khin-dashboards"></a>
...
<!-- TOC --><a name="37-templates-and-template-groups"></a>
...
<!-- TOC --><a name="371-cu-hình-template"></a>
...
<!-- TOC --><a name="372-nhóm-template-template-groups"></a>
...
<!-- TOC --><a name="38-alerts"></a>
### 3.8 Alerts 
<!-- TOC --><a name="38-alerts-1"></a>
### 3.8 Alerts 
...
<!-- TOC --><a name="381-các-loi-phng-tin-media-types"></a>
...
<!-- TOC --><a name="382-hành-ng-actions"></a>
...
<!-- TOC --><a name="39-users-and-user-groups"></a>
...
<!-- TOC --><a name="392-quyn-hn-permissions"></a>
...
<!-- TOC --><a name="393-nhóm-ngi-dùng-user-groups"></a>
...
<!-- TOC --><a name="310-scheduled-reports"></a>
...
<!-- TOC --><a name="5-discovery"></a>
## 5. Discovery
<!-- TOC --><a name="51-network-discovery"></a>
...
<!-- TOC --><a name="513-configuring-a-network-discovery-rule"></a>
...
<!-- TOC --><a name="52-active-agent-autoregistration"></a>
...
<!-- TOC --><a name="53-low-level-discovery-lld"></a>
...
<!-- TOC --><a name="61-sao-lu-c-s-d-liu-database-backups"></a>
...
<!-- TOC --><a name="62-sao-lu-tp-cu-hình-configuration-files"></a>
...
<!-- TOC --><a name="63-sao-luxut-cu-hình-thông-qua-frontendapi-importexport-files"></a>
...
<!-- TOC --><a name="71-use-certificate"></a>
### 7.1 Use Certificate 
...
<!-- TOC --><a name="72-pre-shared-keys-psk"></a>
...
<!-- TOC --><a name="8-tunning"></a>
## 8. Tunning 
<!-- TOC --><a name="81-ti-u-hóa-mysqlmariadb-cho-zabbix"></a>
...
<!-- TOC --><a name="811-kt-ni-và-lung-connections-and-threads"></a>
...
<!-- TOC --><a name="812-cu-hình-innodb-innodb-configuration"></a>
...
<!-- TOC --><a name="813-hiu-sut-và-ti-u-hóa-performance-and-optimization"></a>
...
<!-- TOC --><a name="814-thi-gian-ch-timeouts"></a>
...
<!-- TOC --><a name="815-b-m-và-gii-hn-caches-and-limits"></a>
...
<!-- TOC --><a name="9-high-availability"></a>
## 9. High Availability
<!-- TOC --><a name="91-proxy-group"></a>
### 9.1 Proxy Group
<!-- TOC --><a name="911-tng-quan-v-proxy-load-balancing"></a>
...
<!-- TOC --><a name="912-cu-hình-mt-nhóm-proxy"></a>
...
<!-- TOC --><a name="92-zabbix-server-high-availability"></a>
### 9.2 Zabbix Server High Availability 
<!-- TOC --><a name="921-tng-quan"></a>
#### 9.2.1 Tổng quan
...
<!-- TOC --><a name="922-cu-hình-thit-lp-zabbix-server-ha"></a>
...
- Cấu hình Frontend (Tự động phát hiện node active):
	...
	// $ZBX_SERVER_PORT = '';
	```
<!-- TOC --><a name="923-qun-lý-node-ha"></a>
...
<!-- TOC --><a name="923-các-tin-trình-ha-và-chuyn-i-d-phòng-failover"></a>
...
<!-- TOC --><a name="924-cu-hình-agent-và-proxy-cho-ha"></a>
...
- Cấu hình Zabbix Proxy:
	...
	- Proxy chủ động (Active Proxy):Tham số `Server` phải liệt kê tất cả các node trong cụm được phân tách bằng dấu chấm phẩy.
		- Ví dụ: `Server=10.1.1.31;10.1.1.32`
<!-- TOC --><a name="10-labs"></a>
## 10. Labs
<!-- TOC --><a name="101-cài-t-zabbix-74-trên-ubuntu-2204"></a>
### 10.1 Cài đặt Zabbix 7.4 trên Ubuntu 22.04  
- Hướng dẫn này sẽ chỉ cách cài đặt Zabbix 7.4 trên hệ điều hành Ubuntu 22.04 
- Bước 1: Cập Nhật Hệ Thống
	- Trước khi bắt đầu, điều quan trọng là phải đảm bảo hệ thống được cập nhật hoàn toàn. Điều này giúp tránh các xung đột về phần mềm và đảm bảo có các bản vá bảo mật mới nhất.
		```bash
		sudo apt update && sudo apt upgrade -y
		```
		- `sudo apt update`: Lệnh này sẽ cập nhật danh sách các gói có sẵn từ kho phần mềm.
		- `sudo apt upgrade -y`: Lệnh này sẽ nâng cấp tất cả các gói đã cài đặt lên phiên bản mới nhất. Tuỳ chọn `  -y ` tự động đồng ý với tất cả các câu hỏi xác nhận.
		- ![images](./images/zb-1.png)
- Bước 2: Cài Đặt MariaDB 11.2
	- Zabbix cần một cơ sở dữ liệu để lưu trữ dữ liệu giám sát. Chúng ta sẽ sử dụng MariaDB, một hệ quản trị cơ sở dữ liệu phổ biến và mạnh mẽ. Zabbix 7.4 yêu cầu MariaDB phiên bản nhỏ hơn 11.8, nên sẽ cài đặt phiên bản 11.2.
	- Đầu tiên, cần thêm kho lưu trữ (repository) của MariaDB để có thể cài đặt phiên bản cụ thể.
	```bash
	curl -LsS https://downloads.mariadb.com/MariaDB/mariadb_repo_setup | sudo bash -s -- --mariadb-server-version=11.2
	```
	- Lệnh này tải xuống một script và chạy nó để thêm kho lưu trữ MariaDB 11.2 vào hệ thống.
	- Tiếp theo, cập nhật lại danh sách gói và cài đặt MariaDB server và client:
		```bash
		sudo apt update
		sudo apt -y install mariadb-server mariadb-client
		```
		- `sudo apt update`: Cập nhật lại danh sách gói để hệ thống nhận diện kho lưu trữ MariaDB mới.
		- `sudo -y install mariadb-server mariadb-client`: Cài đặt gói MariaDB server (máy chủ cơ sở dữ liệu) và MariaDB client (công cụ để tương tác với cơ sở dữ liệu).
		- ![images](./images/zb-6.png)
		
	- Bảo mật Cài Đặt MariaDB
		- Sau khi cài đặt, cần bảo mật MariaDB bằng cách chạy công cụ `mariadb-secure-installation`. Công cụ này sẽ giúp thiết lập mật khẩu root, xóa người dùng ẩn danh, cấm đăng nhập root từ xa và xóa cơ sở dữ liệu kiểm tra.
		```bash
		sudo mariadb-secure-installation
		```
		- Enter current password for root (enter for none): Nhấn `Enter` vì hiện tại chưa có mật khẩu root.
		- Switch to unix\_socket authentication [Y/n]: Nhấn `n` (No) để có thể đặt mật khẩu root riêng.
		- Change the root password? [Y/n]: Nhấn `Y` (Yes) và đặt một mật khẩu mạnh cho người dùng root của MariaDB. Hãy ghi nhớ mật khẩu này\!
		- Remove anonymous users? [Y/n]: Nhấn `Y` (Yes).
		- Disallow root login remotely? [Y/n]: Nhấn `Y` (Yes) để tăng cường bảo mật.
		- Remove test database and access to it? [Y/n]: Nhấn `Y` (Yes).
		- Reload privilege tables now? [Y/n]: Nhấn `Y` (Yes).
	
- Bước 3: Khởi Tạo Cơ Sở Dữ Liệu và Người Dùng cho Zabbix
	- Bây giờ, chúng ta sẽ tạo một cơ sở dữ liệu và một người dùng riêng cho Zabbix trong MariaDB. Điều này giúp Zabbix có nơi để lưu trữ dữ liệu và có quyền truy cập cần thiết.
	- Đăng nhập vào MariaDB với tư cách người dùng root (root) vừa đặt mật khẩu:
		```bash
		mysql -u root -p
		```
		- Nhập mật khẩu root MariaDB khi được hỏi.
	- Sau khi đăng nhập thành công vào MariaDB (dấu nhắc lệnh sẽ thay đổi thành `MariaDB [(none)]>`), hãy chạy các lệnh sau từng dòng một:
		```sql
		create database zabbix character set utf8mb4 collate utf8mb4_bin;
		create user zabbix@localhost identified by 'Qaz@123';
		grant all privileges on zabbix. to zabbix@localhost;
		flush privileges;
		quit;
		```
		- `create database zabbix ...`: Tạo một cơ sở dữ liệu mới có tên `zabbix` với bộ ký tự và đối chiếu phù hợp.
		- `create user zabbix@localhost identified by 'Qaz@123'`: Tạo một người dùng mới có tên `zabbix` chỉ có thể kết nối từ `localhost` và đặt mật khẩu là `Qaz@123`. Lưu ý: Nên thay đổi mật khẩu này thành một mật khẩu mạnh hơn trong môi trường sản xuất.
		- `grant all privileges on zabbix. to zabbix@localhost`: Cấp tất cả các quyền trên cơ sở dữ liệu `zabbix` cho người dùng `zabbix@localhost`.
		- `flush privileges;`: Làm mới các quyền để những thay đổi có hiệu lực ngay lập tức.
		- `quit;`: Thoát khỏi MariaDB.
		- ![images](./images/zb-7.png)
		
- Bước 4: Tải và Cài Đặt Kho Lưu Trữ Zabbix
	- Để cài đặt Zabbix, cần thêm kho lưu trữ Zabbix chính thức vào hệ thống. Điều này sẽ cho phép cài đặt các gói Zabbix.
		```bash
		wget https://repo.zabbix.com/zabbix/7.4/release/ubuntu/pool/main/z/zabbix-release/zabbix-release_latest_7.4+ubuntu22.04_all.deb
		sudo dpkg -i zabbix-release_latest_7.4+ubuntu22.04_all.deb
		sudo apt update
		```
		- `wget ...`: Tải xuống gói cài đặt kho lưu trữ Zabbix.
		- `sudo dpkg -i ...`: Cài đặt gói đã tải xuống, thêm kho lưu trữ Zabbix vào hệ thống.
		- `sudo apt update`: Cập nhật lại danh sách gói để hệ thống nhận diện kho lưu trữ Zabbix mới.
		- ![images](./images/zb-2.png)
		- ![images](./images/zb-3.png)
		- ![images](./images/zb-4.png)
		
- Bước 5: Cài Đặt Zabbix Server, Frontend và Agent
	- Bây giờ sẽ cài đặt các thành phần chính của Zabbix: Zabbix Server, Zabbix Frontend (giao diện web để tương tác với Zabbix), và Zabbix Agent (phần mềm thu thập dữ liệu trên các máy muốn giám sát).
		```bash
		sudo apt install zabbix-server-mysql zabbix-frontend-php zabbix-nginx-conf zabbix-sql-scripts zabbix-agent -y
		```
		- `zabbix-server-mysql`: Gói máy chủ Zabbix với hỗ trợ MariaDB/MySQL.
		- `zabbix-frontend-php`: Giao diện web Zabbix được viết bằng PHP.
		- `zabbix-nginx-conf`: Cấu hình Nginx cho Zabbix frontend.
		- `zabbix-sql-scripts`: Chứa các script SQL để tạo cơ sở dữ liệu Zabbix.
		- `zabbix-agent`: Zabbix Agent để giám sát chính máy chủ Zabbix.
		- ![images](./images/zb-5.png)
		
- Bước 6: Nhập Dữ Liệu vào Cơ Sở Dữ Liệu Zabbix
	- Zabbix cần các bảng và dữ liệu ban đầu trong cơ sở dữ liệu `zabbix` để hoạt động. Sẽ nhập các script SQL được cung cấp sẵn.
	- Trước khi nhập, cần tạm thời cho phép `log_bin_trust_function_creators` trong MariaDB. Điều này cần thiết để nhập các hàm.
	- Đăng nhập lại vào MariaDB với tư cách người dùng root:
		```bash
		mysql -u root -p
		```
		- Nhập mật khẩu root MariaDB khi được hỏi.
	- Trong MariaDB, chạy lệnh sau:
		```sql
		set global log_bin_trust_function_creators = 1;
		quit;
		```
		- `set global log_bin_trust_function_creators = 1;`: Tạm thời cho phép tạo các hàm trong nhật ký nhị phân.
		- `quit;`: Thoát khỏi MariaDB.
	- Bây giờ, nhập dữ liệu vào cơ sở dữ liệu Zabbix:
		```bash
		zcat /usr/share/zabbix/sql-scripts/mysql/server.sql.gz | mysql --default-character-set=utf8mb4 -u zabbix -p zabbix -h localhost
		```
		- `zcat ... | mysql ...`: Lệnh này giải nén file SQL nén và "pipe" (chuyển) nội dung của nó trực tiếp vào cơ sở dữ liệu `zabbix` bằng người dùng `zabbix`.
		- Cần nhập mật khẩu của người dùng `zabbix`.
		- ![images](./images/zb-8.png)
		
	- Sau khi nhập xong, hãy quay lại MariaDB và đặt lại `log_bin_trust_function_creators` về 0 để tăng cường bảo mật:
		```bash
		mysql -u root -p
		```
		- Nhập mật khẩu root MariaDB khi được hỏi.
	- Trong MariaDB, chạy lệnh sau:
		...
		quit;
		```
		- ![images](./images/zb-9.png)
		
- Bước 7: Cấu Hình Zabbix Server và Nginx
	- Bây giờ, cần cấu hình Zabbix Server để biết cách kết nối với cơ sở dữ liệu và cấu hình Nginx để phục vụ giao diện web Zabbix.
	- Cấu hình thông số kết nối cơ sở dữ liệu cho Zabbix Server
		- Mở file cấu hình Zabbix Server bằng trình soạn thảo `nano`:
		```bash
		sudo nano /etc/zabbix/zabbix_server.conf
		```
		- Tìm dòng `DBPassword=` (nó có thể bị chú thích bằng dấu `#` ở đầu). Bỏ chú thích (xóa dấu `#`) và đặt mật khẩu đã sử dụng cho người dùng `zabbix` trong Bước 3.
		```
		DBPassword=Qaz@123
		```
		- Lưu ý: Đảm bảo rằng đã sử dụng mật khẩu chính xác đã đặt cho người dùng `zabbix` (`Qaz@123` trong ví dụ này).
		- Nhấn `Ctrl + X`, sau đó `Y`, sau đó `Enter` để lưu và thoát file.
		- ![images](./images/zb-10.png)
		
	- Cấu hình PHP cho Zabbix Frontend với Nginx
		- Mở file cấu hình Nginx cho Zabbix frontend:
		```bash
		...
		```
		- Bỏ chú thích (xóa dấu `#`) ở đầu các dòng này và thay đổi `example.com` thành địa chỉ IP hoặc tên miền của máy chủ Zabbix. Nếu không chắc chắn, có thể để trống hoặc sử dụng `localhost` nếu chỉ truy cập từ cùng một máy chủ.
			- Ví dụ:
			```nginx
			- listen 8080;
			- server_name your_server_ip_or_domain; # Ví dụ: 192.168.1.100 hoặc zabbix.yourdomain.com
			```
			- Lưu ý: `listen 8080;` có nghĩa là Zabbix frontend sẽ khả dụng trên cổng 8080. Có thể thay đổi cổng này nếu muốn.
			- Nếu chỉ muốn truy cập bằng địa chỉ IP của máy chủ, có thể đặt `server_name` thành địa chỉ IP đó.
			- Nhấn `Ctrl + X`, sau đó `Y`, sau đó `Enter` để lưu và thoát file.
		- ![images](./images/zb-11.png)
			
- Bước 8: Khởi Động và Kích Hoạt Các Dịch Vụ Zabbix
	- Cuối cùng, cần khởi động lại các dịch vụ Zabbix Server, Zabbix Agent, Nginx và PHP-FPM để các thay đổi có hiệu lực và đảm bảo chúng tự động khởi động cùng hệ thống.
	```bash
	sudo systemctl restart zabbix-server zabbix-agent nginx php8.1-fpm
	sudo systemctl enable zabbix-server zabbix-agent nginx php8.1-fpm
	```
	- `sudo systemctl restart ...`: Khởi động lại các dịch vụ đã liệt kê.
	- `sudo systemctl enable ...`: Đảm bảo rằng các dịch vụ này sẽ tự động khởi động mỗi khi hệ thống khởi động.
	- ![images](./images/zb-13.png)
	
- Bước 9: Truy Cập Giao Diện Web Zabbix
	- Bây giờ, Zabbix đã được cài đặt và cấu hình. Có thể truy cập giao diện web của Zabbix bằng trình duyệt web.
	- Mở trình duyệt và nhập địa chỉ sau:
		- `http://your_server_ip_or_domain:8080`
		- (Thay `your_server_ip_or_domain` bằng địa chỉ IP hoặc tên miền của máy chủ Zabbix, ví dụ: `http://192.168.1.100:8080`).
	- Sẽ thấy trang chào mừng của Zabbix frontend. Hãy làm theo các bước sau để hoàn tất quá trình thiết lập:
	- ![images](./images/zb-14.png)
	- 1. Welcome: Nhấn `Next step`.
	- 2. Check of pre-requisites: Đảm bảo tất cả các mục đều có trạng thái `OK`. Nhấn `Next step`.
	- ![images](./images/zb-15.png)
	- 3. Configure DB connection:
		- Database type: `MySQL`
		- Host: `localhost`
		- Port: `0` (hoặc `3306` nếu đã thay đổi)
		- Database name: `zabbix`
		- User: `zabbix`
		- Password: `Qaz@123` (hoặc mật khẩu đã đặt)
		- Nhấn `Next step`.
	- ![images](./images/zb-16.png)
	- ![images](./images/zb-17.png)
	- 4.  Pre-installation summary: Xem lại các thiết lập. Nhấn `Next step`.
	- ![images](./images/zb-18.png)
	- 5.  Install: Zabbix sẽ tiến hành cài đặt. Sau khi hoàn tất, nhấn `Finish`.
	- ![images](./images/zb-19.png)
	- Hoàn tất cấu hình chuyển đến trang đăng nhập Zabbix.
	- Tên người dùng (Username): `Admin`
	- Mật khẩu (Password): `zabbix`
	- ![images](./images/zb-20.png)
	- ![images](./images/zb-21.png)

- Zabbix Server đã được cài đặt thành công. Bây giờ có thể bắt đầu cấu hình các thiết bị và dịch vụ để giám sát.

 
<!-- TOC --><a name="102-các-thao-tác-vi-zabbix"></a>
### 10.2 Các thao tác  với Zabbix 
<!-- TOC --><a name="1021-giám-sát-c-bn"></a>
#### 10.2.1 Giám sát cơ bản
- Thông tin về các host được cấu hình trong Zabbix có sẵn trong phần Thu thập dữ liệu \> Hosts cũng như Giám sát \> Hosts. 
- Để thêm một host mới, nhấp vào Create host. Thao tác này sẽ hiển thị một biểu mẫu cấu hình host.
- Các mục tối thiểu cần nhập tại đây bao gồm:
	- Host name: Nhập tên host. Cho phép các ký tự chữ và số, dấu cách, dấu chấm, dấu gạch ngang và dấu gạch dưới.
	- Host groups: Chọn một hoặc một vài nhóm hiện có bằng cách nhấp vào nút Select hoặc nhập tên nhóm không tồn tại để tạo một nhóm mới.
	- Interfaces: IP address: Mặc dù về mặt kỹ thuật đây không phải là một trường bắt buộc, nhưng một giao diện host là cần thiết để thu thập một số số liệu nhất định. Để sử dụng kiểm tra thụ động của Zabbix agent, hãy chỉ định IP hoặc DNS của agent trong trường này. Lưu ý rằng cũng cần chỉ định IP hoặc DNS của Zabbix server trong chỉ thị 'Server' của tệp cấu hình Zabbix agent. Nếu Zabbix agent và Zabbix server được cài đặt trên cùng một máy, cần chỉ định cùng một IP/DNS ở cả hai nơi.
- Ví dụ tạo host db1 - 192.168.50.130
	- ![images](./images/z-20.png)
	- Host này chạy Linux cài đặt MariaDB.
		- Template: Có thể gán Linux server, MySQL via agent2.
		- ![images](./images/z-21.png)
		- ![images](./images/z-22.png)
		- Host group: Database và Linux.
		- ![images](./images/z-23.png)	
		- Interfaces: Cấu hình IP của máy giám sát.
		- ![images](./images/z-24.png)
		- Description: Mô tả host.
		- Monitored by: Cấu hình máy chủ sẽ giám sát sử dụng server / proxy / proxy group. Cơ bản nhất là sử dụng server.
		- ![images](./images/z-25.png)
		- Enabled: Tích chọn để bật host.
	- Nhấp Add để thêm.
...
- Cài đặt Agent 
	...
	```
	- Cài đặt Zabbix agent 2 plugins
	```bash
	# apt install zabbix-agent2-plugin-mssql
	```
	- Cấu hình
	- Mở tệp cấu hình Zabbix agent 2:
	```bash
	...
	# Cấu hình IP của máy chủ Zabbix, Zabbix Proxy (nếu sử dụng). Nếu có nhiều server thì cách nhau bởi dấu phẩy `,`
	Server=192.168.50.140
	##### Active checks related
	# Cấu hình IP của máy chủ Zabbix, Zabbix Proxy (nếu sử dụng). Nếu có nhiều server thì cách nhau bởi dấu chấm phẩy `;`
	ServerActive=192.168.50.140
	```
	- ![images](./images/z-29.png)
	- ![images](./images/z-30.png)
	- Khởi động lại Zabbix agent 2
	```bash
...
- Kiểm tra
	- Kiểm tra Host trạng thái Availability đã thay đổi 
	- ![images](./images/z-33.png)
	- Kiểm tra lastest data có dữ liệu 
	- ![images](./images/z-32.png)

<!-- TOC --><a name="1022-to-nhn-cnh-báo"></a>
#### 10.2.2 Tạo, nhận cảnh báo 
- Zabbix hỗ trợ nhiều nền tảng nhận cảnh báo 
- Ví dụ cấu hình gửi cảnh báo qua Telegram 
- Tạo Bot trên Telegram
	- Để khởi tạo quá trình gửi cảnh báo từ Zabbix đến Telegram, việc đầu tiên là tạo một Telegram Bot để tiếp nhận các thông báo. Thực hiện các bước sau:
	- Tìm kiếm BotFather: Mở ứng dụng Telegram, sử dụng chức năng tìm kiếm và chọn "BotFather" từ kết quả hiển thị.
	- Khởi tạo Bot mới: Gửi tin nhắn /newbot cho BotFather.
	- Đặt tên Bot: Làm theo hướng dẫn của BotFather để đặt tên cho Bot. Sau khi hoàn tất, BotFather sẽ cung cấp một mã truy cập (token) cho Bot. Mã này là cần thiết cho các bước cấu hình tiếp theo.
	- Lưu trữ mã truy cập: Ghi lại mã truy cập được cung cấp.
	- Kích hoạt Bot: Truy cập vào liên kết t.me/username_bot (thay thế username_bot bằng tên người dùng của Bot vừa tạo) và nhấn "Start" để kích hoạt Bot.
	- ![images](./images/z-34.png)
- Lấy ID nhóm (Tùy chọn cho cảnh báo nhóm)
	- Nếu cần gửi cảnh báo đến một nhóm Telegram, thực hiện các bước sau để lấy ID nhóm:
	- Thêm Bot "myidbot" vào nhóm: Thêm @myidbot vào nhóm Telegram mong muốn.
	- Lấy ID nhóm: Trong nhóm, gửi tin nhắn /getgroupid@myidbot. Bot sẽ phản hồi với ID của nhóm.
	- Thêm Bot của cảnh báo vào nhóm: Thêm Bot Telegram vừa tạo ở trên vào nhóm này.
	- ![images](./images/z-35.png)
- Cấu hình Media Types trong Zabbix
	- Sau khi có mã truy cập của Bot Telegram, tiến hành cấu hình trong giao diện quản trị Zabbix:
	- Đăng nhập và điều hướng: Đăng nhập vào giao diện quản trị Zabbix.
	- Chọn Media Types: Điều hướng đến mục Alerts > Media types.
	- Chọn Telegram: Tìm và chọn mục Telegram trong danh sách các loại Media.
		- ![images](./images/z-36.png)
		- Nhập Bot Token: Trong trường "Bot token" (Mã truy cập bot), dán mã truy cập đã lưu.
		- Cấu hình Parse Mode: Chọn chế độ phân tích cú pháp cho tin nhắn (Parse Mode): markdown, markdownv2, hoặc html tùy theo yêu cầu định dạng.
		- Lưu cấu hình: Nhấp vào nút Update để lưu các thay đổi.
	- Kích hoạt Media Type: Trong cột "Status", chọn Enabled cho Telegram để kích hoạt Media Type này.
	- ![images](./images/z-37.png)
	
- Cấu hình Người dùng (Users)
	- Tiếp theo, cấu hình người dùng Zabbix để nhận cảnh báo qua Telegram:
	- Chọn người dùng: Điều hướng đến mục Users > Users và chọn người dùng cần cấu hình cảnh báo.
	- Thêm Media mới: Chọn thẻ Media và nhấp vào Add để thêm một Media mới.
		- Chọn Type: Trong phần "Type", chọn loại Media đã tạo trước đó là "Telegram".
		- Điền Chat ID: Tại trường "Send to", điền Chat ID của người dùng hoặc ID của nhóm đã có. 
		- Cấu hình thời gian hoạt động: Tại "When active", nhập giới hạn khung thời gian mà cảnh báo có thể được gửi (ví dụ: 1-7,00:00-24:00).
	- ![images](./images/z-38.png)
	- Lưu thay đổi: Nhấp vào nút Update để lưu Media mới.
	- ![images](./images/z-39.png)
	- Cập nhật thông tin người dùng: Cuối cùng, nhấp vào Update để lưu thông tin thay đổi của người dùng.
- Tạo Action
	- Tạo Action để định nghĩa hành vi gửi cảnh báo khi có sự kiện Trigger:
	- Điều hướng đến Action: Chọn mục Alerts > Action > Trigger actions.
	
	- Tạo Action mới: Nhấp vào Create action.
		- ![images](./images/z-40.png)
		- Đặt tên: Đặt tên cho Action.
		- Cấu hình điều kiện (Tùy chọn): Nếu cần, cấu hình các điều kiện để kích hoạt Action (ví dụ: mức độ nghiêm trọng của sự kiện, nhóm host, v.v.).
		- ![images](./images/z-44.png)
		- Cấu hình Operations: Chọn thẻ Operations > Operations và nhấp vào Add.
		- Chọn người nhận cảnh báo: Cấu hình nhóm người dùng hoặc chỉ định người dùng cụ thể sẽ nhận được cảnh báo.
		- Sent to Media Type: Có thể để mặc định để Zabbix gửi tới tất cả các loại Media đã được cấu hình, hoặc chỉ định "Telegram" nếu muốn chỉ gửi qua Telegram.
		- ![images](./images/z-41.png)
		- Tùy chỉnh tin nhắn (Tùy chọn): Sử dụng tùy chọn "Custom message" để tùy chỉnh nội dung tin nhắn cảnh báo.
		- ![images](./images/z-42.png)
		- Cấu hình Recovery/Update: Thực hiện tương tự cho các phần "Recovery operations" (khi sự cố được khắc phục) và "Update operations" (khi có cập nhật trạng thái sự cố) nếu cần.
		- ![images](./images/z-43.png)
	- Hoàn tất: Nhấp vào Add để hoàn tất việc tạo Action.
		- ![images](./images/z-45.png)
	
- Kiểm tra khi có trigger sẽ có thông báo gửi tới group trên Telegram
- ![images](./images/z-46.png)
	
<!-- TOC --><a name="1023-giám-sát-service"></a>
#### 10.2.3 Giám sát Service  
- Tính năng giám sát dịch vụ của Zabbix cho phép định nghĩa tình trạng của các hệ thống quan trọng bằng cách tổng hợp trạng thái của từng thành phần.
- Điều này đặc biệt hữu ích cho các thiết lập tính sẵn sàng cao (HA) như cụm Zabbix Server.
- Tạo dịch vụ cha `Zabbix Server HA`
	- Tạo một dịch vụ cha đại diện cho tình trạng tổng thể của cụm Zabbix Server HA.
	1.  Điều hướng đến **Zabbix** -> **Services** -> **Services**.
	2.  Nhấn **Edit** để chuyển sang chế độ chỉnh sửa.
		- ![images](./images/zb-101.png)
	3.  Nhấn **Create New Service**.
	4.  **Tên:** Nhập `Zabbix Server HA`.
		- ![images](./images/zb-102.png)
	5.  **Cấu hình nâng cao:**
		* Trong "Problem rules", đặt điều kiện là **"Disaster" nếu có ít nhất 2 dịch vụ con gặp sự cố**. Điều này đảm bảo rằng dịch vụ HA chỉ báo cáo "Disaster" nếu nhiều node gặp sự cố, phản ánh tính dự phòng.
		- ![images](./images/zb-104.png)
	6.  **Tags:**
		* Nhấn "Add" bên cạnh "Tags".
		* Nhập `Service` vào trường "Tag" và `Zabbix Server HA` vào trường "Value". Tag này sẽ được sử dụng sau này để cấu hình SLA.
		- ![images](./images/zb-103.png)
	7.  Nhấn **Add** để tạo dịch vụ mới.
-  Tạo dịch vụ cho từng node thuộc cụm
	- Tạo các dịch vụ riêng lẻ cho từng node trong cụm Zabbix Server HA và liên kết chúng với dịch vụ cha `Zabbix Server HA`.
	1.  Khi vẫn ở **Zabbix** -> **Services** -> **Services**, nhấn **Create service**.
	2.  **Tên:** Cung cấp một tên mô tả cho từng node (ví dụ: `Zabbix Server Node 1`, `Zabbix Server Node 2`).
		- ![images](./images/zb-105.png)
		- ![images](./images/zb-106.png)
	3.  **Dịch vụ cha:** Chọn `Zabbix Server HA` từ danh sách thả xuống.
	4.  Cấu hình các dịch vụ riêng lẻ để giám sát các tiến trình hoặc máy chủ Zabbix cụ thể cho node đó. Điều này có thể bao gồm việc liên kết với các máy chủ hiện có hoặc tạo mới.
	5.  Nhấn **Add** để tạo dịch vụ cho node.
	6.  Lặp lại các bước này cho mọi node trong cụm Zabbix Server.

- Cấu hình SLA `Zabbix Server HA`
	- Thỏa thuận mức dịch vụ (SLA) trong Zabbix cho phép định nghĩa và theo dõi các mục tiêu về tính sẵn sàng cho các dịch vụ.
	1.  Điều hướng đến **Zabbix** -> **Services** -> **SLA**.
	2.  Nhấn **Create SLA**.
		- ![images](./images/zb-108.png)
	3.  **Tên:** Nhập `Zabbix Server HA`.
	4.  **SLO (Mục tiêu mức dịch vụ):** Đặt là `95%`.
	5.  **Reporting period:** Cấu hình khoảng thời gian báo cáo mong muốn (ví dụ: hàng ngày, hàng tuần, hàng tháng).
	6.  **Service Tag:** Cấu hình giá trị tag đã tạo ở phần Services.
		* **Services Equals:** Chọn `Zabbix Server HA`.
		- ![images](./images/zb-109.png)
	7.  Nhấn **Add** để tạo SLA.
		- ![images](./images/zb-110.png)
		
- Kiểm tra báo cáo SLA
	- Sau khi cấu hình, có thể kiểm tra báo cáo SLA để xem hiệu suất dịch vụ.
	- Điều hướng đến **Zabbix** -> **Services** -> **SLA report**.
		- ![images](./images/zb-111.png)
	- Báo cáo này cung cấp thông tin về SLI (Chỉ số mức dịch vụ), thời gian hoạt động (uptime), thời gian ngừng hoạt động (downtime) và lỗi của dịch vụ đã cấu hình theo dõi.

<!-- TOC --><a name="1024-giám-sát-web"></a>
#### 10.2.4 Giám sát Web  
- Tính năng Giám sát kịch bản web cho phép người dùng kiểm tra hiệu suất và khả năng hoạt động của các ứng dụng web một cách hiệu quả.
	- Cách thức hoạt động:
	- Kịch bản web có thể gán trực tiếp vào máy chủ hoặc gán vào mẫu (template). Việc gán vào mẫu đặc biệt hữu ích khi cần áp dụng cùng một kịch bản cho nhiều máy chủ, giúp tiết kiệm thời gian và công sức.
	- Những thông tin thu thập được:
		- Mỗi kịch bản web sẽ cung cấp cái nhìn tổng quan về tình trạng ứng dụng với các thông tin sau:
		- Tốc độ tải xuống trung bình: Hiển thị hiệu suất tổng thể của toàn bộ kịch bản.
		- Bước lỗi: Xác định chính xác bước nào trong kịch bản gặp sự cố.
		- Thông báo lỗi cuối cùng: Cung cấp chi tiết về nguyên nhân gây ra lỗi.
- Ngoài ra, từng bước riêng lẻ trong kịch bản web cũng được giám sát chặt chẽ, thu thập các thông tin chi tiết như:
		- Tốc độ tải xuống mỗi giây: Đánh giá hiệu suất của từng hành động cụ thể.
		- Thời gian phản hồi: Đo lường độ trễ khi thực hiện một hành động.
		- Mã phản hồi: Cho biết trạng thái của phản hồi từ máy chủ (ví dụ: 200 OK, 404 Not Found).
- Lưu trữ và sử dụng dữ liệu:
	- Tất cả dữ liệu thu thập từ việc thực thi kịch bản web sẽ được lưu trữ an toàn trong cơ sở dữ liệu. Dữ liệu này sau đó tự động được sử dụng để:
	- Vẽ biểu đồ: Trực quan hóa xu hướng hiệu suất theo thời gian.
	- Kích hoạt cảnh báo: Tự động thông báo khi có sự cố xảy ra.
	- Gửi thông báo: Cảnh báo người quản trị về các vấn đề cần chú ý.
- Tính năng này giúp người dùng chủ động theo dõi và xử lý các vấn đề liên quan đến ứng dụng web, đảm bảo trải nghiệm người dùng luôn mượt mà.
- Ví dụ : Giám sát trang chủ Wikipedia tiếng Việt 
	- Truy cập tạo kịch bản web mới
		- 1.  Trong giao diện Zabbix, điều hướng đến Data Collections > Hosts.
		- 2.  Chọn một Host hoặc Template cần gán kịch bản web.
		- ![images](./images/z-82.png)
		- 3.  Click vào tab Web scenarios, sau đó click vào nút Create web scenario.
		- ![images](./images/z-83.png)
	- Cấu hình Kịch bản tổng thể
		- Tại màn hình "Web scenario", điền các thông tin sau:
		- Name:`Wiki Check`
		- Update interval:`1m` (1 phút) – Zabbix sẽ kiểm tra kịch bản này mỗi 1 phút.
		- Agent:`Chrome`
		- Attempts:`1` (Zabbix sẽ chỉ thử một lần duy nhất. Nếu thất bại, nó sẽ báo lỗi ngay).
		- ![images](./images/z-84.png)
		
	-  Thêm các bước (Steps) vào kịch bản
		- Kịch bản này chỉ có một bước duy nhất: Truy cập trang chủ Wikipedia tiếng Việt và kiểm tra chuỗi nội dung.
		- 1.  Trong phần "Steps" của màn hình cấu hình kịch bản web, click vào Add.
		- 2.  Điền thông tin cho Step 1:
			- Name:`Wikipedia Check`
			- URL:`https://vi.wikipedia.org/wiki/Trang_Ch%C3%ADnh`
			- Required status codes:`200` (Zabbix sẽ coi bước này thành công nếu nhận được mã trạng thái HTTP 200 OK).
			- Required string:`Bạn chính là tác giả của Wikipedia` (Đây là một câu đặc trưng thường xuất hiện ở đầu trang chủ Wikipedia tiếng Việt. có thể mở trang này trong trình duyệt để tìm một chuỗi khác nếu muốn). Việc tìm chuỗi này giúp xác nhận rằng không chỉ trang tải mà còn tải đúng nội dung dự kiến.
			- Timeout:`15s` (Nếu trang không phản hồi trong 10 giây, bước này sẽ bị coi là thất bại).
		- ![images](./images/z-92.png)
		- 3.  Click Add để lưu bước này.
	- Lưu kịch bản web: Sau khi đã cấu hình xong, click vào nút Add ở dưới cùng màn hình để lưu toàn bộ kịch bản web.
		- ![images](./images/z-86.png)
	
	- Xem dữ liệu giám sát
		- Sau khi kịch bản được lưu và Zabbix bắt đầu thu thập dữ liệu, có thể xem kết quả:
			- 1.  Điều hướng đến Monitoring> Latest data.
			- 2.  Trong bộ lọc, chọn Host mà đã gán kịch bản web.
			- 3.  Tìm kiếm các item có tên bắt đầu bằng `Web scenario "Wikipedia Check":`. sẽ thấy các thông tin như:
			- `Web scenario "Wiki Check": Failed step` (Sẽ là 0 nếu thành công).
			- `Web scenario "Wiki Check": Last error message` (Sẽ trống nếu thành công).
			- `Web scenario "Wiki Check": Download speed per second for scenario` (Tốc độ tải xuống tổng thể).
			- `Web scenario "Wiki Check": Step "Wiki Check" response time` (Thời gian phản hồi của trang).
			- `Web scenario "Wiki Check": Step "Wiki Check" response code` (Mã phản hồi HTTP, ví dụ: 200).
			- ![images](./images/z-93.png)
		- Xem Đồ thị kết quả 
			- Điều hướng đến Monitoring> Hosts
			- Tại Host đã cấu hình chọn Web 
			- ![images](./images/z-94.png)
			- Chọn Web scenario
			- ![images](./images/z-95.png)
			- Graph về Tốc độ, Thời gian phản hồi 
			- ![images](./images/z-90.png)
			
	- Tạo Trigger cảnh báo
		- Để Zabbix cảnh báo khi trang Wikipedia có vấn đề, cần tạo một Trigger.
		- 1.  Điều hướng đến Data Collections > Hosts> Chọn Host cấu hình > tab Triggers.
		- ![images](./images/z-96.png)
		- 2.  Click Create trigger.
		- ![images](./images/z-97.png)
		- 3.  Cấu hình trigger:
			- Name:`Wikipedia Error
			- Severity:`High` 
			- Expression:Click Add và tìm item `Web scenario "Wikipedia Check": Failed step`.
			- ![images](./images/z-98.png)
			- Chọn hàm `last()` và đặt điều kiện là `=1` (chỉ có một bước, nên nếu nó lỗi thì Failed step sẽ là 1).
			- ![images](./images/z-89.png)
			- Click Add để lưu trigger.
		- Khi kịch bản thất bại, giá trị của item `Web scenario "Wikipedia Check": Failed step` sẽ chuyển từ `0` sang `1`.
		- Điều này sẽ khớp với điều kiện `{...}.last()=1` trong trigger, và Zabbix sẽ ngay lập tức kích hoạt cảnh báo, thông báo cho về sự cố.
		- Cảnh báo về telegram 
			- ![images](./images/z-91.png)
		
<!-- TOC --><a name="1025-discovery"></a>
#### 10.2.5 Discovery  
- Network Discovery giúp tự động tìm kiếm các thiết bị và dịch vụ trong mạng.
- Điều này đặc biệt hữu ích để tự động thêm các host mới, giám sát các dịch vụ đang chạy trên chúng, và phản ứng với các thay đổi trong hạ tầng mạng mà không cần cấu hình thủ công.
- Ví dụ thực tế: Tự động phát hiện Host và giám sát dịch vụ SSH1
	- Trên dải mạng (ví dụ: 192.168.50.0/24) nơi các máy chủ Linux được triển khai.
	- Mục tiêu là để Zabbix tự động phát hiện chúng và bắt đầu giám sát trạng thái của dịch vụ SSH trên mỗi máy.
- Tạo quy tắc phát hiện (Discovery Rule)
	- Trong giao diện Zabbix, điều hướng đến Data Collections > Discovery.
	- Nhấp vào nút Create discovery rule.
	- ![images](./images/z-99.png)
- Cấu hình Quy tắc phát hiện
	- Điền các thông tin sau:
	- Name: Linux SSH Discovery
	- Discover by: Cấu hình việc discover thực hiện bởi Server hay Proxy 
	- IP range: `192.168.50.1-254`. Đây là dải địa chỉ Zabbix sẽ quét.
	- Update Interval: Cấu hình thời gian Zabbix sẽ chạy quy tắc.
	- Checks: Các kiểm tra mà Zabbix sẽ thực hiện trên mỗi IP trong dải để xác định loại thiết bị hoặc dịch vụ.
		- Nhấp Add dưới phần "Checks".
		- Type: SSH (Service)
		- Port range: 22 
		- ![images](./images/z-100.png)
	- Nhấp Add để lưu Quy tắc phát hiện.
	- ![images](./images/z-101.png)
	- ![images](./images/z-102.png)

- Tạo hành động phát hiện (Discovery Action)
	- Đây là phần quan trọng nhất: Định nghĩa Zabbix sẽ làm gì khi một thiết bị phù hợp với quy tắc phát hiện được tìm thấy.
	- Trong giao diện Zabbix, sau khi tạo quy tắc phát hiện, chuyển tới Alerts -> Actions -> Discovery Actions 
	- ![images](./images/z-103.png)
	- Nhấp vào nút Create action.
	- ![images](./images/z-104.png)
	- Cấu hình Hành động phát hiện
	- Tab "Action"
	- Name: Add Host Discovery
	- Conditions: Các điều kiện mà một thiết bị được phát hiện phải đáp ứng để hành động này được thực hiện.
		- Nhấp Add dưới phần "Conditions".
			- Condition type: Discovery rule
			- Operator: =
			- Value: Chọn quy tắc vừa tạo.
			- Nhấp Add.
		- ![images](./images/z-105.png)
		- Thêm một điều kiện nữa:
			- Condition type: Service type
			- Operator: =
			- Value: SSH
			- Nhấp Add.
		- ![images](./images/z-106.png)
	- Tab "Operations"
		- Đây là những gì Zabbix sẽ làm khi các điều kiện trên được đáp ứng.
		- Nhấp Add dưới phần "Operations".
		- Operation type: Add host
		- Nhấp Add.
		- ![images](./images/z-107.png)
		- Operation type: Link to templates
		- Templates: Tìm và chọn template phù hợp để giám sát dịch vụ. 
		- Nhấp Add.
		- Nhấp Add để lưu Hành động.
		- ![images](./images/z-108.png)
		- ![images](./images/z-109.png)
		- ![images](./images/z-110.png)

- Xem kết quả phát hiện
	- Sau khi các quy tắc và hành động đã được cấu hình, Zabbix sẽ bắt đầu quá trình phát hiện theo Interval đã đặt.
	- Điều hướng đến Monitoring > Discovery.
	- Sẽ thấy danh sách các thiết bị đã được phát hiện, trạng thái của chúng
	- Nếu có máy chủ Linux nào trong dải IP đó có cổng 22 mở, chúng sẽ xuất hiện ở đây.
	- ![images](./images/z-111.png)
	- Sau khi hành động phát hiện được thực thi, các host mới đó sẽ xuất hiện trong Configuration > Hosts và được gán các template đã chọn.


<!-- TOC --><a name="1026-bo-mt"></a>
#### 10.2.6 Bảo mật
<!-- TOC --><a name="10261-s-dng-psk"></a>
##### 10.2.6.1 Sử dụng PSK
- Mặc định, Zabbix Agent và Server giao tiếp dưới dạng văn bản thuần túy. Để mã hóa, có thể sử dụng tùy chọn mã hóa dựa trên PSK.
- PSK bao gồm hai giá trị quan trọng: PSK identity và PSK Secret.
	- PSK identity: Tên định danh cho Agent, được cấu hình trên cả Agent và Server.
	- PSK Secret: Khóa bí mật dùng để mã hóa và giải mã dữ liệu. Khóa này phải có độ dài tối thiểu 128-bit (16 byte PSK, nhập dưới dạng 32 chữ số thập lục phân) đến tối đa 2048-bit (256 byte PSK, nhập dưới dạng 512 chữ số thập lục phân).

- Cấu hình trên Zabbix Agent
	- Thực hiện các bước sau trên máy chủ Zabbix Agent:
	- Sinh và lưu trữ PSK
	- Sinh một khóa PSK 256-bit (32 byte) bằng OpenSSL, sau đó lưu khóa vào một tệp và cấu hình quyền truy cập.
		```bash
		sudo sh -c 'KEY=$(openssl rand -hex 32); echo "$KEY" | tee /etc/zabbix/zabbix_agentd.psk > /dev/null; chown zabbix:zabbix /etc/zabbix/zabbix_agentd.psk; chmod 400 /etc/zabbix/zabbix_agentd.psk; echo "Generated PSK: $KEY"'
		```
		- Giải thích lệnh:
			- `openssl rand -hex 32`: Sinh một chuỗi thập lục phân ngẫu nhiên dài 32 byte (tương đương 256 bit).
			- `echo "$KEY" | tee /etc/zabbix/zabbix_agentd.psk > /dev/null`: Lưu chuỗi PSK đã sinh vào tệp `/etc/zabbix/zabbix_agentd.psk`.
			- `chown zabbix:zabbix /etc/zabbix/zabbix_agentd.psk`: Gán quyền sở hữu tệp cho người dùng và nhóm `zabbix`.
			- `chmod 400 /etc/zabbix/zabbix_agentd.psk`: Đặt quyền truy cập tệp chỉ cho phép chủ sở hữu đọc.
			- `echo "Generated PSK: $KEY"`: Hiển thị PSK đã sinh ra. Lưu giữ chuỗi PSK này để cấu hình trên Zabbix Server.
		- ![images](./images/zb-56.png)
	- Chỉnh sửa cấu hình Zabbix Agent
		- Mở tệp cấu hình Zabbix Agent:
		```bash
		nano /etc/zabbix/zabbix_agentd.conf
		```
		- Thêm các dòng sau trong tệp cấu hình:
		```
		TLSConnect=psk
		TLSAccept=psk
		TLSPSKFile=/etc/zabbix/zabbix_agentd.psk
		TLSPSKIdentity=PSK_DB1
		```
		- Giải thích các tham số:
			- `TLSConnect=psk`: Zabbix Agent sẽ sử dụng TLS với PSK khi chủ động kết nối tới Zabbix Server.
			- `TLSAccept=psk`: Zabbix Agent chỉ chấp nhận các kết nối đến từ Zabbix Server nếu chúng sử dụng PSK.
			- `TLSPSKFile=/etc/zabbix/zabbix_agentd.psk`: Chỉ định đường dẫn đến tệp chứa chuỗi PSK (đã sinh ở trên).
			- `TLSPSKIdentity=PSK_DB1`: Đặt tên định danh cho Agent. Tên này phải trùng khớp với tên sẽ được cấu hình trên Zabbix Server. Có thể thay đổi `PSK_DB1` thành một tên định danh khác tùy ý.
		- ![images](./images/zb-57.png)
	- Khởi động lại Zabbix Agent
		- Sau khi chỉnh sửa cấu hình, khởi động lại dịch vụ Zabbix Agent để áp dụng các thay đổi:
		```bash
		sudo service zabbix-agent restart
		```
		- ![images](./images/zb-58.png)
		
- Cấu hình trên giao diện Zabbix Server
	- Truy cập giao diện người dùng Zabbix Server và cấu hình các tùy chọn mã hóa PSK cho máy chủ tương ứng.
	- Điều hướng đến cấu hình Host
		- 1. Đăng nhập vào giao diện Zabbix Server.
		- 2. Truy cập `Data Collection` \> Hosts.
		- 3. Chọn Host (máy chủ Agent) cần cấu hình.
		- 4. Chuyển đến tab Encryption.
	- Cấu hình tùy chọn mã hóa
		- Trong tab Encryption, cấu hình các tùy chọn sau:
		- Connections to host: Chọn `PSK`.
		- Connections from host: Chọn `PSK`.
		- PSK Identity: Nhập chính xác tên định danh đã sử dụng trong cấu hình Zabbix Agent (`TLSPSKIdentity`), ví dụ: `PSK_DB1`.
		- PSK: Dán chuỗi thập lục phân dài của PSK đã sinh ra từ lệnh OpenSSL ở trên vào trường này.
	- ![images](./images/zb-59.png)
	- Lưu cấu hình
		- Nhấp vào nút Update để lưu các thay đổi.
- Kiểm tra kết nối
	- Sau khi hoàn tất cấu hình trên cả Agent và Server, Zabbix Server và Agent sẽ bắt đầu giao tiếp bằng mã hóa PSK. Quá trình này có thể mất một hoặc hai phút để thiết lập. Kiểm tra trạng thái của Host trong giao diện Zabbix Server để đảm bảo kết nối đã được thiết lập thành công.
	- ![images](./images/zb-60.png)

<!-- TOC --><a name="10262-s-dng-certificate"></a>
##### 10.2.6.2 Sử dụng Certificate 
- Zabbix có khả năng sử dụng chứng chỉ RSA định dạng PEM, được ký bởi một tổ chức cấp chứng chỉ (CA) công cộng hoặc nội bộ. 
- Việc xác minh chứng chỉ được thực hiện dựa trên chứng chỉ CA đã được cấu hình trước.
- Cấu hình bảo mật truyền thông giữa Zabbix Agent và Zabbix Server sử dụng chứng chỉ SSL/TLS. 
- Quá trình này bao gồm việc tạo chứng chỉ, cấu hình Zabbix Server, Zabbix Proxy và Zabbix Agent, cùng với việc cập nhật cấu hình trên giao diện Zabbix Frontend. 
- Ví dụ cấu hình sử dụng chứng chỉ tự ký Self-Signed
<!-- TOC --><a name="a-to-chng-ch"></a>
###### a. Tạo chứng chỉ
- Tạo thư mục
```bash
mkdir zabbix
cd zabbix/
```
- Tạo khóa riêng và chứng chỉ cho Root CA:
	```bash
	openssl genrsa -out ca.key 4096
	openssl req -x509 -new -nodes -key ca.key -sha256 -days 3650 -out ca.crt -subj "/C=VN/ST=Hanoi/L=Hanoi/O=ZabbixCA/OU=annt/CN=Root-CA"
	```
	- `ca.key`: Khóa riêng của Root CA.
	- `ca.crt`: Chứng chỉ của Root CA.
	- `/C=VN/ST=Hanoi/L=Hanoi/O=ZabbixCA/OU=annt/CN=Root-CA`: Thông tin về Chủ thể (Subject) của chứng chỉ Root CA.
- Tạo khóa riêng và yêu cầu ký chứng chỉ (CSR) cho Zabbix Server:
	```bash
	openssl genrsa -out zabbix_servers.key 2048
	openssl req -new -key zabbix_servers.key -out zabbix_servers.csr -subj "/C=VN/ST=Hanoi/L=Hanoi/O=Zabbix/OU=annt/CN=zabbix-servers"
	```
	- `zabbix_servers.key`: Khóa riêng của Zabbix Server.
	- `zabbix_servers.csr`: Yêu cầu ký chứng chỉ của Zabbix Server.
	- `/C=VN/ST=Hanoi/L=Hanoi/O=Zabbix/OU=annt/CN=zabbix-servers`: Thông tin về Chủ thể (Subject) của chứng chỉ Zabbix Server.
- Ký chứng chỉ cho Zabbix Server bằng Root CA:
	```bash
	openssl x509 -req -in zabbix_servers.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out zabbix_servers.crt -days 1095 -sha256
	```
	- `zabbix_servers.crt`: Chứng chỉ đã ký của Zabbix Server.
	- `Certificate request self-signature ok`: Xác nhận quá trình ký thành công.
	- `subject=C = VN, ST = Hanoi, L = Hanoi, O = Zabbix, OU = annt, CN = zabbix-servers`: Thông tin Chủ thể của chứng chỉ đã ký.
- Tạo khóa riêng và yêu cầu ký chứng chỉ (CSR) cho Zabbix Proxies:
	```bash
	openssl genrsa -out zabbix_proxies.key 2048
	openssl req -new -key zabbix_proxies.key -out zabbix_proxies.csr -subj "/C=VN/ST=Hanoi/L=Hanoi/O=Zabbix/OU=annt/CN=zabbix-proxies"
	```
	- `zabbix_proxies.key`: Khóa riêng của Zabbix Proxy.
	- `zabbix_proxies.csr`: Yêu cầu ký chứng chỉ của Zabbix Proxy.
	- `/C=VN/ST=Hanoi/L=Hanoi/O=Zabbix/OU=annt/CN=zabbix-proxies`: Thông tin về Chủ thể (Subject) của chứng chỉ Zabbix Proxy.
- Ký chứng chỉ cho Zabbix Proxies bằng Root CA:
	```bash
	openssl x509 -req -in zabbix_proxies.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out zabbix_proxies.crt -days 1095 -sha256
	```
	- `zabbix_proxies.crt`: Chứng chỉ đã ký của Zabbix Proxy.
	- `Certificate request self-signature ok`: Xác nhận quá trình ký thành công.
	- `subject=C = VN, ST = Hanoi, L = Hanoi, O = Zabbix, OU = annt, CN = zabbix-proxies`: Thông tin Chủ thể của chứng chỉ đã ký.
- Tạo khóa riêng và yêu cầu ký chứng chỉ (CSR) cho Zabbix Agents:
	```bash
	openssl genrsa -out zabbix_agents.key 2048
	openssl req -new -key zabbix_agents.key -out zabbix_agents.csr -subj "/C=VN/ST=Hanoi/L=Hanoi/O=Zabbix/OU=annt/CN=zabbix-agents"
	```
	- `zabbix_agents.key`: Khóa riêng của Zabbix Agent.
	- `zabbix_agents.csr`: Yêu cầu ký chứng chỉ của Zabbix Agent.
	- `/C=VN/ST=Hanoi/L=Hanoi/O=Zabbix/OU=annt/CN=zabbix-agents`: Thông tin về Chủ thể (Subject) của chứng chỉ Zabbix Agent.
- Ký chứng chỉ cho Zabbix Agents bằng Root CA:
	```bash
	openssl x509 -req -in zabbix_agents.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out zabbix_agents.crt -days 1095 -sha256
	```
	- `zabbix_agents.crt`: Chứng chỉ đã ký của Zabbix Agent.
	- `Certificate request self-signature ok`: Xác nhận quá trình ký thành công.
	- `subject=C = VN, ST = Hanoi, L = Hanoi, O=Zabbix, OU=annt, CN=zabbix-agents`: Thông tin Chủ thể của chứng chỉ đã ký.
- Cấu hình truy cập cho các khóa riêng:
	```bash
	chmod 400 `.key
	```
	- Lệnh này đảm bảo chỉ người dùng sở hữu mới có quyền đọc các tệp khóa riêng, tăng cường bảo mật.
- Kiểm tra danh sách các tệp đã tạo:
	```bash
	ls -l
	```
	- Kết quả hiển thị tương tự như sau:
	```
	-rw-r--r-- 1 root root 2017 Jul 30 01:57 ca.crt
	-r-------- 1 root root 3268 Jul 30 01:56 ca.key
	-rw-r--r-- 1 root root 1554 Jul 30 01:58 zabbix_agents.crt
	-rw-r--r-- 1 root root 1001 Jul 30 01:58 zabbix_agents.csr
	-r-------- 1 root root 1704 Jul 30 01:58 zabbix_agents.key
	-rw-r--r-- 1 root root 1554 Jul 30 01:58 zabbix_proxies.crt
	-rw-r--r-- 1 root root 1001 Jul 30 01:58 zabbix_proxies.csr
	-r-------- 1 root root 1704 Jul 30 01:58 zabbix_proxies.key
	-rw-r--r-- 1 root root 1554 Jul 30 01:57 zabbix_servers.crt
	-rw-r--r-- 1 root root 1001 Jul 30 01:57 zabbix_servers.csr
	-r-------- 1 root root 1704 Jul 30 01:57 zabbix_servers.key
	```
	- ![images](./images/zb-69.png)
- Lấy thông tin Issuer và Subject từ chứng chỉ:
	- Để có được thông tin Issuer và Subject của một chứng chỉ, sử dụng lệnh sau (thay thế `<Certificate>` bằng tên tệp chứng chỉ tương ứng):
	```bash
	openssl x509 -noout -issuer -subject -nameopt esc_2253,esc_ctrl,utf8,dump_nostr,dump_unknown,dump_der,sep_comma_plus,dn_rev,sname -in <Certificate>
	```
	- `Ví dụ cho `ca.crt`:`
	```bash
	openssl x509 -noout -issuer -subject \
		...
		-in ca.crt
	```
	- Kết quả sẽ hiển thị Issuer và Subject của chứng chỉ đó.
	- ![images](./images/zb-68.png)
<!-- TOC --><a name="b-cu-hình-zabbix-server"></a>
###### b. Cấu hình Zabbix Server
- Tạo thư mục lưu trữ chứng chỉ trên Zabbix Server:
	```bash
	mkdir /etc/zabbix/cert
	```
- Sao chép các tệp chứng chỉ cần thiết tới Zabbix Server:
	- Sao chép các tệp sau vào thư mục `/etc/zabbix/cert` trên Zabbix Server:
	- `ca.crt`
	- `zabbix_servers.crt`
	- `zabbix_servers.key`
	
- Chỉnh sửa cấu hình Zabbix Server:
	...
	```
	TLSCAFile=/etc/zabbix/cert/ca.crt
	TLSCertFile=/etc/zabbix/cert/zabbix_servers.crt
	TLSKeyFile=/etc/zabbix/cert/zabbix_servers.key
	```
	- ![images](./images/zb-64.png)
	- `TLSCAFile`: Đường dẫn đầy đủ đến tệp chứa chứng chỉ CA cấp cao nhất để xác minh chứng chỉ ngang hàng.
	- `TLSCertFile`: Đường dẫn đầy đủ đến tệp chứa chứng chỉ.
	- `TLSKeyFile`: Đường dẫn đầy đủ đến tệp chứa khóa riêng. Đảm bảo tệp này chỉ có thể đọc được bởi người dùng Zabbix bằng cách đặt quyền truy cập thích hợp.
	- Lưu và đóng tệp. 
- Khởi động lại dịch vụ Zabbix Server:
```bash
systemctl restart zabbix-server
```
<!-- TOC --><a name="c-cu-hình-zabbix-proxy"></a>
###### c. Cấu hình Zabbix Proxy 
- Tạo thư mục lưu trữ chứng chỉ trên Zabbix Proxy:
```bash
mkdir /etc/zabbix/cert
```
- Sao chép các tệp chứng chỉ cần thiết tới Zabbix Proxy:
	- Sao chép các tệp sau vào thư mục `/etc/zabbix/cert` trên Zabbix Proxy:
	- `ca.crt`
	- `zabbix_proxies.crt`
	- `zabbix_proxies.key`
- Chỉnh sửa cấu hình Zabbix Proxy:
	- Mở tệp cấu hình Zabbix Proxy:
	```bash
	...
	TLSAccept=cert 
	TLSCAFile=/etc/zabbix/cert/ca.crt 
	TLSServerCertIssuer=CN=Root-CA,OU=annt,O=ZabbixCA,L=Hanoi,ST=Hanoi,C=VN 
	TLSServerCertSubject=CN=zabbix-servers,OU=annt,O=Zabbix,L=Hanoi,ST=Hanoi,C=VN 
	TLSCertFile=/etc/zabbix/cert/zabbix_proxies.crt 
	TLSKeyFile=/etc/zabbix/cert/zabbix_proxies.key 
	```
	- ![images](./images/zb-65.png)
	- Lưu và đóng tệp.
- Lưu ý:
	- Để cải thiện bảo mật của proxy, có thể đặt các tham số `TLSServerCertIssuer` và `TLSServerCertSubject`.
	- `TLSServerCertIssuer` và `TLSServerCertSubject` được sử dụng để hạn chế các chứng chỉ được phép kết nối. Giá trị này phải khớp với Issuer và Subject của chứng chỉ Zabbix Server (`zabbix_servers.crt`).
	- Thông tin Issuer và Subject có thể lấy bằng lệnh `openssl x509 -noout -issuer -subject -in <Certificate>`.
- Khởi động lại dịch vụ Zabbix Proxy:
```bash
systemctl restart zabbix-proxy
```
- Cấu hình mã hóa cho Zabbix Proxy trên Zabbix Frontend:
	- Truy cập giao diện Zabbix Frontend.
	- Điều hướng đến: `Administration` → `Proxies`.
	- Chọn Zabbix Proxy cần cấu hình và nhấp vào tab `Encryption`.
	- Cấu hình như sau:
		- Đối với Active Proxy:
			- `Connection to proxy`: `No encryption` 
			- `Connection from proxy`: `Cert` 
			- `Issuer`: `CN=Root-CA,OU=annt,O=ZabbixCA,L=Hanoi,ST=Hanoi,C=VN` (Issuer của `ca.crt`) 
			- `Subject`: `CN=zabbix-proxies,OU=annt,O=Zabbix,L=Hanoi,ST=Hanoi,C=VN` (Subject của `zabbix_proxies.crt`) 
		- Đối với Passive Proxy:
			- `Connection to proxy`: `Cert` 
			- `Connection from proxy`: `No encryption` 
			- `Issuer`: `CN=Root-CA,OU=annt,O=ZabbixCA,L=Hanoi,ST=Hanoi,C=VN` (Issuer của `ca.crt`) 
			- `Subject`: `CN=zabbix-proxies,OU=annt,O=Zabbix,L=Hanoi,ST=Hanoi,C=VN` (Subject của `zabbix_proxies.crt`) 
	- ![images](./images/zb-66.png)

<!-- TOC --><a name="d-cu-hình-zabbix-agent"></a>
###### d. Cấu hình Zabbix Agent
- Tạo thư mục lưu trữ chứng chỉ trên Zabbix Agent:
```bash
mkdir /etc/zabbix/cert
```
- Sao chép các tệp chứng chỉ cần thiết tới Zabbix Agent:
	- Sao chép các tệp sau vào thư mục `/etc/zabbix/cert` trên Zabbix Agent:
	- `ca.crt`
	- `zabbix_agents.crt`
	- `zabbix_agents.key`
- Chỉnh sửa cấu hình Zabbix Agent:
	- Mở tệp cấu hình Zabbix Agent (ví dụ: `zabbix_agentd.conf` hoặc `zabbix_agent2.conf`):
	```bash
	nano /etc/zabbix/zabbix_agent2.conf
	```
	- Tìm và chỉnh sửa các tham số sau:
	...
	TLSCAFile=/etc/zabbix/cert/ca.crt 
	TLSServerCertIssuer=CN=Root-CA,OU=annt,O=ZabbixCA,L=Hanoi,ST=Hanoi,C=VN 
	TLSServerCertSubject=CN=zabbix-proxies,OU=annt,O=Zabbix,L=Hanoi,ST=Hanoi,C=VN 
	TLSCertFile=/etc/zabbix/cert/zabbix_agents.crt 
	TLSKeyFile=/etc/zabbix/cert/zabbix_agents.key 
	```
	- ![images](./images/zb-70.png)
	- Lưu và đóng tệp.
- Lưu ý:
	- Để cải thiện bảo mật của agent, có thể đặt các tham số `TLSServerCertIssuer` và `TLSServerCertSubject`.
	- `TLSServerCertIssuer` và `TLSServerCertSubject` được sử dụng để hạn chế các chứng chỉ được phép kết nối. Giá trị này phải khớp với Issuer và Subject của chứng chỉ Zabbix Server (`zabbix_servers.crt`) hoặc Zabbix Proxy (`zabbix_proxies.crt`) mà Agent kết nối đến.
	- Thông tin Issuer và Subject có thể lấy bằng lệnh `openssl x509 -noout -issuer -subject -in <Certificate>`.
- Khởi động lại dịch vụ Zabbix Agent:
	```bash
	systemctl restart zabbix-agent2
	```
- Cấu hình mã hóa cho Host trên Zabbix Frontend:
	- Truy cập giao diện Zabbix Frontend.
	- Điều hướng đến: `Data collection` → `Hosts`.
	- Chọn Host cần cấu hình và nhấp vào tab `Encryption`.
	- Cấu hình như sau:
		- Đối với Passive Agent:
			- `Connections to host`: `Cert` 
			- `Connections from host`: `No encryption` 
			- `Issuer`: `CN=Root-CA,OU=annt,O=ZabbixCA,L=Hanoi,ST=Hanoi,C=VN` (Issuer của `ca.crt`) 
			- `Subject`: `CN=zabbix-agents,OU=annt,O=Zabbix,L=Hanoi,ST=Hanoi,C=VN` (Subject của `zabbix_agents.crt`) 
		- Đối với Active Agent:
			- `Connections to host`: `No encryption` 
			- `Connections from host`: `Cert` 
			- `Issuer`: `CN=Root-CA,OU=annt,O=ZabbixCA,L=Hanoi,ST=Hanoi,C=VN` (Issuer của `ca.crt`) 
			- `Subject`: `CN=zabbix-agents,OU=annt,O=Zabbix,L=Hanoi,ST=Hanoi,C=VN` (Subject của `zabbix_agents.crt`) 
	- ![images](./images/zb-67.png)

- Sau khi hoàn tất các bước trên, kết nối giữa Zabbix Agent, Zabbix Proxy và Zabbix Server sẽ được bảo mật bằng chứng chỉ SSL/TLS. 
	- Proxy 
	- ![images](./images/zb-71.png)
	- Host 
	- ![images](./images/zb-73.png)
	- Log 
		- Agent 
		- ![images](./images/zb-72.png)
		- Proxy 
		- ![images](./images/zb-74.png)	
<!-- TOC --><a name="1027-giám-sát-s-dng-snmp"></a>
#### 10.2.7 Giám sát sử dụng SNMP 
- Zabbix hỗ trợ giám sát các thiết bị mạng thông qua Giao thức Quản lý Mạng Đơn giản (SNMP). 
- SNMP là một giao thức chuẩn để thu thập thông tin từ các thiết bị mạng như switch, switch, máy in, server, v.v., miễn là chúng có hỗ trợ SNMP agent.
- Ví dụ thực tế: Giám sát một switch Cisco qua SNMP trên Zabbix
	- Chúng ta sẽ cấu hình Zabbix để giám sát trạng thái cổng (interface) và lưu lượng mạng của một switch Cisco thông qua SNMP.
	- Có một switch Cisco đã cấu hình SNMP (ví dụ: SNMPv2c với community string là public).
	- Trong bài lab này do không có switch thực tế thực hiện tạo [giả lập bằng Docker](./Stimulate%20Switch%20using%20Docker.md)
	- Địa chỉ IP của switch là 192.168.50.250
	- Thêm Host mới vào Zabbix
		- Trong giao diện Zabbix, điều hướng đến Data Collections > Hosts.
		- Click vào nút Create host.

	- Cấu hình Host
		- Điền các thông tin cơ bản cho switch:
		- Host name: LAB-SW3750 (Tên định danh cho thiết bị trong Zabbix)
		- Visible name: LAB-SW3750
		- Templates: Chọn template phù hợp với switch đang cấu hình. 
		- Interfaces: Đây là phần quan trọng để Zabbix biết cách kết nối với thiết bị.
		- Click Add dưới phần "Interfaces".
		- Agent type: Chọn SNMP agent.
		- IP address: 192.168.50.250 (Địa chỉ IP của switch).
		- Port: 161 (Cổng SNMP mặc định).
		- SNMP community: {$SNMP_COMMUNITY}. Để Zabbix kết nối được, SNMP community string phải khớp với cài đặt trên switch.
		- ![images](./images/z-112.png)
	- Cấu hình Macro SNMP Community
		- Vì chúng ta đã sử dụng macro {$SNMP_COMMUNITY}, cần định nghĩa giá trị thực cho nó:
		- Chuyển sang tab Macros.
		- Trong phần "Host macros", click Add.
		- Macro: {$SNMP_COMMUNITY}
		- Value: public 
		- Có thể thay thế bằng community string thực tế trên switch 
		- Click Add để lưu macro.
		- ![images](./images/z-113.png)
		
	- Lưu Host: Sau khi đã cấu hình đầy đủ, click vào nút Add (hoặc Update nếu đang chỉnh sửa) ở dưới cùng màn hình để lưu Host.
		- ![images](./images/z-114.png)

	- Kiểm tra dữ liệu giám sát
		- Zabbix sẽ bắt đầu thu thập dữ liệu từ switch thông qua SNMP. Quá trình này có thể mất một vài phút.
		- Điều hướng đến Monitoring > Latest data.
		- Sử dụng bộ lọc để chọn Host LAB-SW3750.
		- ![images](./images/z-115.png)

	-  Trigger và Graph
		- Các template SNMP được Zabbix cung cấp sẵn thường đi kèm với các trigger và graph mặc định rất hữu ích:
		- ![images](./images/z-116.png)
		- ![images](./images/z-117.png)
		- ![images](./images/z-118.png)

<!-- TOC --><a name="103-zabbix-high-availability"></a>
### 10.3 Zabbix High Availability  
- Hệ thống giám sát Zabbix được thiết kế để đảm bảo tính sẵn sàng cao và khả năng mở rộng thông qua việc phân tán các thành phần chính. Kiến trúc này bao gồm nhiều lớp, mỗi lớp đóng một vai trò quan trọng trong việc duy trì hoạt động liên tục và hiệu quả của hệ thống.
<!-- TOC --><a name="1031-tng-quan"></a>
#### 10.3.1 Tổng quan 
<!-- TOC --><a name="10311-thành-phn-và-vai-trò"></a>
##### 10.3.1.1 Thành phần và vai trò
- Mô hình 

```mermaid
graph TD
    %% Cụm HA Zabbix Server (Active/Passive)
    subgraph Zabbix_Server_Cluster[" (Active/Standby)"]
        direction TB
        VIP["Zabbix Server Cluster"]
        S1["Zabbix Server Node 1 "]
        S2["Zabbix Server Node 2 "]
        DB[( Galera Cluster)]
        VIP --> S1
        VIP -. failover .-> S2
        S1 --- DB
    ...
    %% Nhóm Zabbix Proxy
    subgraph Zabbix_Proxy_Group["Zabbix Proxy Group"]
        direction TB
        P1["Zabbix Proxy Node 1"]
        P2["Zabbix Proxy Node 2"]
    end

    %% Proxies gửi dữ liệu về VIP của cụm Server
    P1 --> VIP
    P2 --> VIP
...
- ![images](./images/zha.png)	
- Hệ thống được xây dựng dựa trên các thành phần chính sau, mỗi thành phần có địa chỉ IP và vai trò cụ thể trong mạng 192.168.50.0/24 :

| IP Address         | Hostname                   | Vai trò / Dịch vụ chính                                      |
| ------------------ | -------------------------- | ------------------------------------------------------------ |
| 192.168.50.130 | `db1`                      | MariaDB Galera Cluster – Node 1                              |
| 192.168.50.131 | `db2`                      | MariaDB Galera Cluster – Node 2                              |
| 192.168.50.132 | `db3`                      | MariaDB Galera Cluster – Node 3                              |
| 192.168.50.133 | `maxscale`                 | MaxScale Load Balancer cho Galera Cluster                    |
| 192.168.50.140 | `zabbix`                   | Zabbix Server Cluster – Node 1 (Active/Passive)              |
| 192.168.50.142 | `zabbix2`                  | Zabbix Server Cluster – Node 2 (Active/Passive)              |
| 192.168.50.150 | `zabbixproxy1`             | Zabbix Proxy Group – Node 1                                  |
| 192.168.50.151 | `zabbixproxy2`             | Zabbix Proxy Group – Node 2                                  |
| 192.168.50.200 | `zabbix-fe`                | Zabbix Web Frontend (Nginx + PHP-FPM)                        |

- MariaDB Galera Cluster (192.168.50.130, 192.168.50.131, 192.168.50.132): Cung cấp một cơ sở dữ liệu backend có tính sẵn sàng cao cho Zabbix. Galera Cluster là một cụm MariaDB đồng bộ gần như hoàn toàn, hỗ trợ nhân bản đa master (multi-master replication), cho phép mọi node đều có thể xử lý đọc và ghi. Cluster này tự động kiểm soát thành viên, loại bỏ node bị lỗi và đảm bảo dữ liệu được cập nhật gần như tức thời trên tất cả các node.
- MaxScale (192.168.50.133): Hoạt động như một proxy cơ sở dữ liệu, cung cấp khả năng cân bằng tải và phân chia đọc/ghi (Read/Write split) cho MariaDB Galera Cluster. Tất cả các kết nối từ Zabbix Server và Zabbix Web Frontend đến cơ sở dữ liệu đều đi qua MaxScale, giúp tối ưu hóa hiệu suất và nâng cao tính sẵn sàng.
- Zabbix Server Cluster (192.168.50.140, 192.168.50.142): Đảm bảo tính sẵn sàng cao cho Zabbix Server thông qua chế độ Cluster Mode native của Zabbix 7.4. Hai node hoạt động ở chế độ Active/Passive, tự động chuyển đổi dự phòng khi một node gặp sự cố, duy trì hoạt động liên tục của Zabbix Server.
- Zabbix Proxy Group (192.168.50.150, 192.168.50.151): Cung cấp khả năng thu thập dữ liệu phân tán, cân bằng tải và tự động chuyển đổi dự phòng cho các host được giám sát. Nhóm proxy này phân chia host và tự động chia tải, đồng thời đảm bảo dữ liệu giám sát vẫn được thu thập và đồng bộ về Zabbix Server Cluster ngay cả khi một proxy node bị down.
- Zabbix Web Frontend (192.168.50.200): Giao diện người dùng để cấu hình, quản lý và xem dữ liệu giám sát. Frontend chạy PHP-FPM kết hợp với Nginx/Apache và kết nối trực tiếp tới Zabbix Server Cluster (qua API) và cơ sở dữ liệu (qua MaxScale).
- Zabbix Agent 2: Được cài đặt trên tất cả các node trong hệ thống để thu thập dữ liệu giám sát về chính các node đó, cung cấp khả năng giám sát nội bộ toàn diện.

<!-- TOC --><a name="10312-lung-hot-ng"></a>
##### 10.3.1.2 Luồng hoạt động
- Luồng hoạt động của hệ thống:
	- Luồng dữ liệu từ Zabbix Web Frontend: Giao diện Web Zabbix (192.168.50.200) tương tác với Zabbix Server Cluster (192.168.50.140/142) thông qua API để thực hiện các tác vụ cấu hình và quản lý. Đối với các truy vấn dữ liệu lịch sử hoặc thông tin trực tiếp từ cơ sở dữ liệu, Web Frontend kết nối đến MaxScale (192.168.50.133). MaxScale sau đó định tuyến các truy vấn này đến MariaDB Galera Cluster (192.168.50.130–132), phân phối truy vấn đọc đến các node có sẵn và truy vấn ghi đến node primary hiện tại.
	- Luồng dữ liệu từ Zabbix Proxies: Các Zabbix Proxy (192.168.50.150, 192.168.50.151) thu thập dữ liệu giám sát từ các Zabbix Agent trên các host được gán cho chúng. Dữ liệu này được lưu trữ tạm thời trong cơ sở dữ liệu cục bộ của proxy trước khi gửi về Zabbix Server Cluster thông qua API hoặc các kết nối active. Tính năng Proxy Group cho phép cân bằng tải giữa hai proxy node, đảm bảo rằng các host được phân chia đều và tự động chuyển đổi dự phòng khi một proxy node bị down.
	- Luồng dữ liệu trong MariaDB Galera Cluster và MaxScale: MaxScale (192.168.50.133) đóng vai trò trung gian cho tất cả các kết nối cơ sở dữ liệu từ Zabbix Server và Web Frontend. MaxScale tự động định tuyến truy vấn ghi đến node primary hiện tại của Galera và phân phối truy vấn đọc đến bất kỳ node nào trong Galera Cluster đang ở trạng thái đồng bộ. MariaDB Galera Cluster đảm bảo mọi thay đổi dữ liệu được nhân bản gần như tức thời đến tất cả các node khác, duy trì tính nhất quán và sẵn sàng cao của dữ liệu.
	- Hoạt động HA của Zabbix Server Cluster: Hai node Zabbix Server (192.168.50.140 và 192.168.50.142) hoạt động ở chế độ Active/Passive. Chỉ một node hoạt động tại một thời điểm, xử lý tất cả các tác vụ. Node còn lại ở chế độ standby, sẵn sàng tiếp quản. Zabbix Server 7.4 có cơ chế HA native, tự động phát hiện trạng thái và thực hiện chuyển đổi dự phòng. Giao diện Web Zabbix được cấu hình để tự động phát hiện node Zabbix Server active.
	- Hoạt động HA của Zabbix Proxy Group: Khi một Zabbix Proxy node bị down, Zabbix Server sẽ tự động chuyển các Zabbix Agent đang báo cáo về proxy đó sang proxy node còn lại. Khi proxy bị down được khôi phục, Zabbix Server sẽ tự động cân bằng lại các host giữa các proxy trong nhóm, hoặc giữ nguyên phân chia nếu số lượng host không đủ để chia đều.

<!-- TOC --><a name="1032-trin-khai"></a>
#### 10.3.2 Triển khai 
<!-- TOC --><a name="10321-cài-t-và-cu-hình-mariadb-galera-cluster-và-maxscale"></a>
##### 10.3.2.1 Cài đặt và cấu hình MariaDB Galera Cluster và MaxScale
<!-- TOC --><a name="a-cài-t-và-cu-hình-mariadb-galera-cluster"></a>
###### a. Cài đặt và cấu hình MariaDB Galera Cluster
- Các bước này cần được thực hiện trên tất cả ba node MariaDB Galera Cluster.
- Cài đặt MariaDB và Galera
	- Zabbix yêu cầu một cơ sở dữ liệu để lưu trữ dữ liệu giám sát. Hệ thống này sử dụng MariaDB 11.2, một phiên bản tương thích với Zabbix 7.4 (yêu cầu MariaDB phiên bản nhỏ hơn 11.8).
	- Thêm kho lưu trữ (repository) của MariaDB để có thể cài đặt phiên bản cụ thể:
	```Bash
	curl -LsS <https://downloads.mariadb.com/MariaDB/mariadb_repo_setup> | sudo bash -s -- --mariadb-server-version=11.2  
	```
	- Cập nhật danh sách gói và cài đặt MariaDB server, client cùng với các gói Galera Cluster:
		```Bash
		sudo apt update  
		sudo apt -y install mariadb-server mariadb-client galera-4 galera-arbitrator-4  
		```
		- `sudo apt update` cập nhật danh sách gói để hệ thống nhận diện kho lưu trữ MariaDB mới. 
		- `sudo -y install mariadb-server mariadb-client` cài đặt máy chủ và công cụ client cơ sở dữ liệu MariaDB. 
		- `galera-4` và `galera-arbitrator-4` là các gói cần thiết để thiết lập và quản lý Galera Cluster.
- Bảo mật MariaDB
	- Sau khi cài đặt, cần bảo mật MariaDB bằng cách chạy công cụ mariadb-secure-installation. Công cụ này hỗ trợ thiết lập mật khẩu root, xóa người dùng ẩn danh, cấm đăng nhập root từ xa và xóa cơ sở dữ liệu kiểm tra, tăng cường an ninh cho hệ thống.
		```Bash
		sudo mariadb-secure-installation  
		```
		- Thực hiện theo các cấu hình:
			- Enter current password for root (enter for none): Nhấn Enter vì hiện tại chưa có mật khẩu root.
			- Switch to unix_socket authentication: Nhấn n (No) để có thể đặt mật khẩu root riêng.
			- Change the root password?: Nhấn Y (Yes) và đặt một mật khẩu mạnh cho người dùng root của MariaDB. Mật khẩu này cần được ghi nhớ cẩn thận.
			- Remove anonymous users?: Nhấn Y (Yes).
			- Disallow root login remotely?: Nhấn Y (Yes) để tăng cường bảo mật.
			- Remove test database and access to it?: Nhấn Y (Yes).
			- Reload privilege tables now?: Nhấn Y (Yes).
- Cấu hình Galera Cluster
	- Trên mỗi node Galera (192.168.50.130, 192.168.50.131, 192.168.50.132), chỉnh sửa file cấu hình MariaDB để kích hoạt và cấu hình Galera Cluster.
	- Mở file cấu hình:
	```Bash
	nano /etc/mysql/mariadb.conf.d/50-server.cnf  
	```
	- Tìm dòng bind-address và sửa thành 0.0.0.0 để cho phép kết nối từ mọi địa chỉ IP.
	- ![images](./images/zb-33.png)
	- Thêm phần cấu hình \[galera\] vào cuối file, đảm bảo các tham số `wsrep_node_address` và `wsrep_node_name` được thiết lập tương ứng với từng node :
		- Trên node 1 (192.168.50.130):
		```Ini, TOML
		[galera]
		wsrep_on=ON
		wsrep_provider=/usr/lib/galera/libgalera_smm.so
		wsrep_cluster_address="gcomm://192.168.50.130,192.168.50.131,192.168.50.132"
		binlog_format=row
		default_storage_engine=InnoDB
		innodb_autoinc_lock_mode=2
		wsrep_cluster_name="zabbix_cluster"
		bind-address=0.0.0.0
		wsrep_node_address="192.168.50.130"
		wsrep_node_name="node1"
		wsrep_sst_method=rsync
		```
		- Trên node 2 (192.168.50.131):
		```Ini, TOML
		[galera]
		wsrep_on=ON
		wsrep_provider=/usr/lib/galera/libgalera_smm.so
		...
		wsrep_sst_method=rsync
		```
	- ![images](./images/zb-75.png)
		
- Khởi tạo và kiểm tra Galera Cluster
	- Trước khi khởi tạo cluster, đảm bảo dịch vụ MariaDB đã dừng trên tất cả các node:
	```Bash
	systemctl stop mysql  
	```
	- Trên node đầu tiên (ví dụ: 192.168.50.130), khởi tạo cluster mới. Lưu ý rằng lệnh này sẽ thất bại nếu bất kỳ dịch vụ MariaDB nào vẫn đang chạy trên các node khác :
	```Bash
	galera_new_cluster  
	```
	- Sau khi lệnh trên hoàn tất, kiểm tra số lượng node đã tham gia cluster:
	```Bash
	mariadb -u root -p -e "SHOW STATUS LIKE 'wsrep_cluster_size'"  
	```
	- Kết quả mong đợi sẽ là Value | 1, cho thấy node đầu tiên đã khởi tạo cluster thành công.
	- Để các node còn lại tham gia cluster, chỉ cần khởi động dịch vụ MariaDB trên node 2 (192.168.50.131) và node 3 (192.168.50.132):
	```Bash
	systemctl start mysql  
	```
	- Sau khi khởi động dịch vụ trên cả hai node còn lại, kiểm tra lại trên một node bất kỳ để xác nhận rằng cả ba node đã tham gia cluster:
	```Bash
	mariadb -u root -p -e "SHOW STATUS LIKE 'wsrep_cluster_size'"  
	```
	- ![images](./images/zb-76.png)
	- Kết quả mong đợi sẽ là Value | 3, xác nhận rằng toàn bộ Galera Cluster đã hoạt động đúng cách.
<!-- TOC --><a name="b-cài-t-và-cu-hình-maxscale-trên-19216850133"></a>
###### b. Cài đặt và cấu hình MaxScale (trên 192.168.50.133)
- MaxScale đóng vai trò là bộ cân bằng tải và định tuyến truy vấn cho Galera Cluster, đảm bảo hiệu suất và tính sẵn sàng cho cơ sở dữ liệu của Zabbix.
- Cài đặt MaxScale
	- Trên node MaxScale (192.168.50.133), cài đặt MariaDB client và MaxScale:
	```Bash
	sudo apt -y install mariadb-client maxscale  
	```
	- Cấu hình MariaDB cho MariaDB MaxScale (trên node Galera Cluster)
	- Để MaxScale có thể kết nối và lấy dữ liệu xác thực từ Galera Cluster, cần tạo một người dùng riêng cho MaxScale trên một trong các node Galera Cluster (ví dụ: 192.168.50.130).
	- Đăng nhập vào MariaDB trên node Galera Cluster 192.168.50.130 với quyền root:
	```Bash
	mysql -uroot -p  
	```
	- Nhập mật khẩu root khi được yêu cầu.
	- Sau đó, thực hiện các lệnh SQL sau để tạo người dùng maxscale và cấp các quyền cần thiết. Người dùng này được cấp quyền SELECT trên các bảng hệ thống của mysql và các quyền giám sát nhân bản (REPLICATION SLAVE, BINLOG MONITOR, REPLICATION MASTER ADMIN, SLAVE MONITOR) để MaxScale có thể theo dõi trạng thái của các node Galera và định tuyến truy vấn một cách chính xác.
	```SQL
	CREATE USER 'maxscale'@'192.168.50.133' IDENTIFIED BY '123@';
	GRANT SELECT ON mysql.user TO 'maxscale'@'192.168.50.133';
	GRANT SELECT ON mysql.db TO 'maxscale'@'192.168.50.133';
	GRANT SELECT ON mysql.tables_priv TO 'maxscale'@'192.168.50.133';
	GRANT SELECT ON mysql.roles_mapping TO 'maxscale'@'192.168.50.133';
	GRANT SHOW DATABASES ON *.* TO 'maxscale'@'192.168.50.133';GRANT REPLICATION SLAVE, BINLOG MONITOR, REPLICATION MASTER ADMIN, SLAVE MONITOR ON *.* TO 'maxscale'@'192.168.50.133';
	GRANT SHOW DATABASES, SLAVE MONITOR ON *.* TO 'maxscale'@'192.168.50.133';
	GRANT SELECT ON mysql.* TO 'maxscale'@'192.168.50.133';
	FLUSH PRIVILEGES; 
	quit;  
	```
- Cấu hình MariaDB MaxScale
	- Trên node MaxScale (192.168.50.133), di chuyển file cấu hình MaxScale mặc định để tạo một file mới và chỉnh sửa:
	```Bash
	mv /etc/maxscale.cnf /etc/maxscale.cnf.bak  
	nano /etc/maxscale.cnf  
	```
	- Dán nội dung cấu hình sau vào file maxscale.cnf:
	```Ini, TOML
	[maxscale]
	threads=auto
	admin_host=0.0.0.0
	...
	```
	- ![images](./images/zb-77.png)
	- Phần `[maxscale]` cấu hình chung cho MaxScale, cho phép truy cập giao diện quản trị từ mọi địa chỉ IP trên cổng 8989.
	- Các phần `[server1], [server2], [server3]` định nghĩa ba node MariaDB Galera Cluster mà MaxScale sẽ quản lý, bao gồm địa chỉ IP và cổng kết nối.
	- `[Galera-Monitor]` cấu hình module giám sát galeramon để theo dõi trạng thái của các node Galera, sử dụng người dùng maxscale đã tạo. Tham số disable_master_failback=true ngăn MaxScale tự động chuyển lại master cũ sau khi nó phục hồi, cho phép kiểm soát tốt hơn quá trình chuyển đổi.
	- `[Read-Write-Service]` cấu hình dịch vụ phân chia đọc/ghi, sử dụng router readwritesplit để tự động định tuyến các truy vấn đọc đến các slave và truy vấn ghi đến master.
	- `[Read-Write-Listener]` cấu hình cổng lắng nghe (cổng 3306) mà MaxScale sẽ sử dụng để nhận các kết nối từ ứng dụng (Zabbix Server và Frontend) và chuyển tiếp chúng đến dịch vụ Read-Write-Service.
	- Khởi động MaxScale và kiểm tra
	- Sau khi đã cấu hình xong file maxscale.cnf, khởi động dịch vụ MaxScale:
	```Bash
	systemctl start maxscale  
	```
	- Để kiểm tra trạng thái của MaxScale, có thể sử dụng lệnh sau:
	```Bash
	systemctl status maxscale  
	```
	- Hoặc truy cập giao diện web của MaxScale tại địa chỉ <http://192.168.50.133:8989> sử dụng thông tin đăng nhập mặc định admin/mariadb.
	- ![images](./images/zb-78.png)
	- ![images](./images/zb-79.png)
	- ![images](./images/zb-80.png)
<!-- TOC --><a name="10322-cài-t-và-cu-hình-zabbix-server-cluster"></a>
##### 10.3.2.2 Cài đặt và cấu hình Zabbix Server Cluster 
- Các bước này cần được thực hiện trên cả hai node Zabbix Server.
- Tải và Cài đặt Zabbix Server
	- Trên cả hai node 192.168.50.140 và 192.168.50.142, thực hiện các lệnh sau để tải và cài đặt Zabbix repository, sau đó cập nhật danh sách gói:
	```Bash
	wget https://repo.zabbix.com/zabbix/7.4/release/ubuntu/pool/main/z/zabbix-release/zabbix-release_latest_7.4+ubuntu22.04_all.deb
	dpkg -i zabbix-release_latest_7.4+ubuntu22.04_all.deb
	apt update
	```
	- Các lệnh này đảm bảo hệ thống có thể truy cập các gói Zabbix cần thiết cho quá trình cài đặt.
	- Cài đặt Zabbix Server và Zabbix Agent
	- Sau khi cập nhật repository, cài đặt các gói Zabbix server, Zabbix SQL scripts và Zabbix agent trên cả hai node:
	```Bash
	apt install zabbix-server-mysql zabbix-sql-scripts zabbix-agent -y  
	```
	- zabbix-server-mysql là gói máy chủ Zabbix với hỗ trợ MySQL/MariaDB. zabbix-sql-scripts chứa các tập lệnh SQL cần thiết để khởi tạo cơ sở dữ liệu Zabbix. zabbix-agent được cài đặt để giám sát chính các node Zabbix Server.
	- Import Dữ liệu vào Cơ sở dữ liệu thông qua MaxScale
	- Trước khi import dữ liệu, cần tạo một người dùng và cơ sở dữ liệu cho Zabbix trên Galera Cluster. Kết nối vào MariaDB thông qua MaxScale (sử dụng địa chỉ IP của MaxScale là 192.168.50.133) và thực hiện các lệnh sau :
	```Bash
	mysql -uroot -p -h 192.168.50.133  
	```
	...
	quit;  
	```
	- Tiếp theo, bật biến log_bin_trust_function_creators trong MariaDB để cho phép tạo các hàm được lưu trữ (stored functions) cần thiết cho Zabbix. Việc này được thực hiện tạm thời để đảm bảo quá trình import schema diễn ra suôn sẻ.
	```Bash
	mysql -u root -p -h 192.168.50.133  
	```
	```
	set global log_bin_trust_function_creators = 1;  
	quit  
	```
	- Sau đó, sử dụng lệnh zcat để giải nén và chuyển nội dung của file schema Zabbix vào cơ sở dữ liệu zabbix thông qua MaxScale, sử dụng người dùng zabbix đã tạo :
	```Bash
	zcat /usr/share/zabbix/sql-scripts/mysql/server.sql.gz | mysql --default-character-set=utf8mb4 -u zabbix -p zabbix -h 192.168.50.133  
	```
	- Sau khi import dữ liệu hoàn tất, nên tắt lại biến log_bin_trust_function_creators để tăng cường bảo mật :
	```Bash
	mysql -u root -p -h 192.168.50.133  
	set global log_bin_trust_function_creators = 0;  
	quit  
...
- Cấu hình Zabbix Server
	- Chỉnh sửa file cấu hình Zabbix server /etc/zabbix/zabbix_server.conf trên cả hai node.
	- Cấu hình các tham số kết nối đến cơ sở dữ liệu, trỏ về địa chỉ IP của MaxScale (192.168.50.133). Điều này đảm bảo rằng cả hai node Zabbix Server đều kết nối đến cùng một điểm cuối MaxScale, mà MaxScale lại kết nối đến cùng một cụm Galera.
	```Bash
	nano /etc/zabbix/zabbix_server.conf  
	```
	- Tìm và chỉnh sửa các dòng sau:
	```
	DBHost=192.168.50.133  
	DBName=zabbix  
	DBUser=zabbix  
	DBPassword=Qaz@123 
	```
	- ![images](./images/zb-81.png)
	- Thêm các tham số High Availability (HA) cho từng node Zabbix Server.
		- Trên Node 1 (192.168.50.140):
		```Ini, TOML
		####### High availability cluster parameters #######  
		HANodeName=zabbix-node-01  
		NodeAddress=192.168.50.140:10051  
		```
		- ![images](./images/zb-82.png)
		- Trên Node 2 (192.168.50.142):
		```Ini, TOML
		####### High availability cluster parameters #######  
		HANodeName=zabbix-node-02  
		NodeAddress=192.168.50.142:10051  
		HANodeName là tên định danh duy nhất cho node Zabbix Server trong cluster HA, còn NodeAddress là địa chỉ IP và cổng lắng nghe của node Zabbix Server hiện tại.
		```
		- ![images](./images/zb-83.png)
	- Khởi động và Kiểm tra Zabbix Server
		- Sau khi cấu hình, khởi động lại dịch vụ Zabbix server trên cả hai node :
		```Bash
		systemctl restart zabbix-server  
		```
		- Kiểm tra trạng thái của dịch vụ Zabbix server để đảm bảo nó đang chạy:
		```Bash
		systemctl status zabbix-server  
		```
		- ![images](./images/zb-44.png)
	- Để kiểm tra trạng thái HA của cluster, sử dụng lệnh sau. Lưu ý rằng lệnh này chỉ chạy thành công trên node Zabbix Server đang ở trạng thái active. Node standby sẽ báo lỗi khi chạy lệnh này.
	```Bash
	zabbix_server -R ha_status  
	```
	- ![images](./images/zb-84.png)	
	- Lệnh này sẽ hiển thị thông tin về trạng thái của các node trong Zabbix HA cluster, xác nhận node nào đang hoạt động và node nào đang ở chế độ chờ.
<!-- TOC --><a name="10323-cài-t-và-cu-hình-zabbix-web-frontend-trên-19216850200"></a>
##### 10.3.2.3 Cài đặt và cấu hình Zabbix Web Frontend (trên 192.168.50.200)
- Thực hiện các bước sau trên node Zabbix Web Frontend (192.168.50.200).
- Tải và cài đặt gói Zabbix Frontend
	- Tải và cài đặt gói Zabbix release cho Ubuntu 22.04, sau đó cập nhật danh sách gói :
	```Bash
	wget https://repo.zabbix.com/zabbix/7.4/release/ubuntu/pool/main/z/zabbix-release/zabbix-release_latest_7.4+ubuntu22.04_all.deb
	dpkg -i zabbix-release_latest_7.4+ubuntu22.04_all.deb
	apt update
	```
	- Cài đặt các gói cần thiết cho Zabbix Frontend, bao gồm các tệp PHP, cấu hình Nginx và Zabbix Agent để giám sát chính node này :
	```Bash
	apt install zabbix-frontend-php zabbix-nginx-conf zabbix-sql-scripts zabbix-agent  
	```
- Cấu hình Zabbix Frontend để kết nối với Zabbix Server Cluster và MaxScale
	- Mở tệp cấu hình Zabbix frontend: /etc/zabbix/web/zabbix.conf.php.
	- Đảm bảo rằng các biến $ZBX_SERVER và $ZBX_SERVER_PORT được để trống hoặc được comment. Điều này cho phép Zabbix frontend tự động phát hiện node Zabbix Server đang hoạt động trong cluster HA, thay vì phải cấu hình thủ công một địa chỉ cụ thể.
	```PHP
	// Uncomment and set to desired values to override Zabbix hostname/IP and port.  
	// $ZBX_SERVER = '';  
	// $ZBX_SERVER_PORT = '';  
	```
	- ![images](./images/zb-86.png)
	- Trong cùng tệp /etc/zabbix/web/zabbix.conf.php, cấu hình các thông số kết nối cơ sở dữ liệu để trỏ về MaxScale (192.168.50.133).
	```PHP
	// Zabbix GUI configuration file.  
	$DB['TYPE']				= 'MYSQL';
	$DB['SERVER']			= '192.168.50.133';
	$DB['PORT']				= '0';
	$DB['DATABASE']			= 'zabbix';
	$DB['USER']				= 'zabbix';
	$DB['PASSWORD']			= 'Qaz@123';
	```
	- ![images](./images/zb-85.png)
	- Đảm bảo rằng DBPassword khớp với mật khẩu đã tạo cho người dùng zabbix trên MariaDB Galera Cluster.
	- Cấu hình PHP và Nginx cho Zabbix Frontend
	- Mở tệp cấu hình Nginx cho Zabbix:
	```Bash
	nano /etc/zabbix/nginx.conf  
	```
	- Bỏ comment và đặt giá trị cho các chỉ thị listen và server_name theo yêu cầu. Ví dụ, để lắng nghe trên cổng 80 và sử dụng địa chỉ IP của node Web Frontend :
	```Nginx
	listen 80; # Hoặc cổng khác nếu cần, ví dụ 8080  
	server_name zabbix.annt.cloud; # Hoặc tên miền nếu có  
	```
	- ![images](./images/zb-87.png)
	- Sau khi hoàn tất cấu hình, khởi động lại dịch vụ Nginx và PHP-FPM để áp dụng các thay đổi :
	```Bash
	systemctl restart nginx php8.1-fpm  
	```
	- Các lệnh này khởi động lại máy chủ web Nginx và trình quản lý tiến trình PHP-FPM, cho phép Zabbix frontend hoạt động và phục vụ giao diện người dùng.
<!-- TOC --><a name="10324-cài-t-và-cu-hình-zabbix-proxy-group-trên-19216850150-19216850151"></a>
##### 10.3.2.4 Cài đặt và cấu hình Zabbix Proxy Group (trên 192.168.50.150, 192.168.50.151)
- Các bước này cần được thực hiện trên cả hai node Zabbix Proxy.
- Cài đặt MariaDB cục bộ trên các node Zabbix Proxy
	- Zabbix Proxy cần một cơ sở dữ liệu cục bộ để lưu trữ dữ liệu giám sát tạm thời trước khi gửi về Zabbix Server. Hệ thống này sử dụng MariaDB 11.2, tương tự như các thành phần khác.
	- Thêm kho lưu trữ MariaDB 11.2:
	```Bash
	curl -LsS https://downloads.mariadb.com/MariaDB/mariadb_repo_setup | sudo bash -s -- --mariadb-server-version=11.2
	```
	- Cập nhật danh sách gói và cài đặt MariaDB server và client:
	```Bash
	sudo apt update  
	sudo apt -y install mariadb-server mariadb-client  
	```
	- Sau khi cài đặt, bảo mật MariaDB bằng cách chạy công cụ mariadb-secure-installation, thực hiện các bước tương tự như đã mô tả trong phần cấu hình Galera Cluster.
	```Bash
	sudo mariadb-secure-installation  
	```
	- Mỗi Zabbix Proxy duy trì cơ sở dữ liệu cục bộ riêng để tạm thời lưu trữ dữ liệu đã thu thập trước khi gửi đến Zabbix Server. Thiết kế này cho phép các proxy tiếp tục thu thập dữ liệu ngay cả khi kết nối đến Zabbix Server tạm thời bị mất, cải thiện tính toàn vẹn dữ liệu và giảm tải cho máy chủ trung tâm. Cơ sở dữ liệu cục bộ đóng vai trò như một bộ đệm, đảm bảo tính bền vững của dữ liệu trong các trường hợp gián đoạn mạng hoặc thời gian ngừng hoạt động của Zabbix Server. Thiết kế này nâng cao tính mạnh mẽ của việc thu thập dữ liệu trong các môi trường giám sát phân tán.
- Cài đặt Zabbix Proxy
	- Tải và cài đặt kho lưu trữ Zabbix, sau đó cập nhật danh sách gói :
	```Bash
	wget https://repo.zabbix.com/zabbix/7.4/release/ubuntu/pool/main/z/zabbix-release/zabbix-release_latest_7.4+ubuntu22.04_all.deb
	dpkg -i zabbix-release_latest_7.4+ubuntu22.04_all.deb
	apt update
	```
	- Cài đặt Zabbix Proxy với hỗ trợ MySQL và các tập lệnh SQL cần thiết :
	```Bash
	apt install zabbix-proxy-mysql zabbix-sql-scripts -y  
	```
	- ![images](./images/zb-48.png)
	- Tạo cơ sở dữ liệu cho Zabbix Proxy
	- Đăng nhập vào MariaDB cục bộ với quyền root và tạo cơ sở dữ liệu zabbix_proxy cùng với người dùng zabbix cho proxy :
	```Bash
	mysql -uroot -p  
	```
	```
	...
	QUIT;  
	```
	- ![images](./images/zb-49.png)
	- Import dữ liệu vào cơ sở dữ liệu proxy
	- Sử dụng lệnh cat để chuyển nội dung của file schema proxy vào cơ sở dữ liệu zabbix_proxy :
	```Bash
	cat /usr/share/zabbix/sql-scripts/mysql/proxy.sql | mysql --default-character-set=utf8mb4 -uzabbix -p zabbix_proxy  
	```
	- Sau khi import dữ liệu, tắt tùy chọn log_bin_trust_function_creators để tăng cường bảo mật :
	```Bash
	mysql -uroot -p  
	SET GLOBAL log_bin_trust_function_creators = 0;  
	QUIT;  
	```
	- ![images](./images/zb-50.png)
	
- Cấu hình Zabbix Proxy
	- Chỉnh sửa file cấu hình Zabbix Proxy /etc/zabbix/zabbix_proxy.conf để cấu hình kết nối cơ sở dữ liệu cục bộ và kết nối tới Zabbix Server Cluster :
	```Bash
	nano /etc/zabbix/zabbix_proxy.conf  
	```
	- Cấu hình kết nối cơ sở dữ liệu cục bộ:
	```
	DBPassword=Qaz@123 # Thay 'password' bằng mật khẩu đã đặt  
	```
	- ![images](./images/zb-51.png)
	
	- Cấu hình kết nối với Zabbix Server Cluster. Server trỏ đến địa chỉ IP của cả hai node Zabbix Server Cluster, phân tách bằng dấu chấm phẩy, đảm bảo proxy có thể kết nối đến server active.
	- Trên node proxy 1 (192.168.50.150):
	```
	Server=192.168.50.140;192.168.50.142  
	Hostname=zabbixproxy1 
	```
	- Trên node proxy 2 (192.168.50.151):
	```
	Server=192.168.50.140;192.168.50.142  
	Hostname=zabbixproxy2 
	```
	- Khởi động và bật Zabbix Proxy
	- Khởi động tiến trình Zabbix Proxy và cấu hình để nó tự động khởi động cùng hệ thống :
	```Bash
	systemctl restart zabbix-proxy  
	systemctl enable zabbix-proxy  
	```
	- ![images](./images/zb-52.png)
	
- Cấu hình Zabbix GUI để tạo Proxy Group
	- Truy cập giao diện Zabbix Web Frontend (<http://192.168.50.200>) và thực hiện các bước sau :
	- 1. Tạo Proxy Group mới:
		- Đăng nhập vào Zabbix GUI.
		- Điều hướng đến Zabbix -> Administration -> Proxy Groups.
		- Nhấn vào nút Create proxy group.
		- Đặt tên cho nhóm proxy, ví dụ: zabbix_proxy_group.
		- Nhấn Add để lưu.
		- ![images](./images/zb-88.png)
	- 2. Thêm các Zabbix Proxy vào nhóm:
		- Điều hướng đến Zabbix -> Administration -> Proxies.
		- Nhấn vào nút Create proxy.
		- Đối với Zabbix Proxy 1 (192.168.50.150):
			- Đặt tên cho proxy, ví dụ: zabbixproxy1 (phải khớp với Hostname đã cấu hình trong zabbix_proxy.conf).
			- Chọn Proxy group là zabbix_proxy_group (nhóm vừa tạo).
			- Chọn Proxy mode là Active proxy.
			- Nhập Proxy address là 192.168.50.150.
			- Nhấn Add để thêm proxy.
			- ![images](./images/zb-89.png)
		- Đối với Zabbix Proxy 2 (192.168.50.151):
			- Lặp lại các bước trên.
			- Đặt tên cho proxy, ví dụ: zabbixproxy2.
			- Chọn Proxy group là zabbix_proxy_group.
			- Chọn Proxy mode là Active proxy.
			- Nhập Proxy address là 192.168.50.151.
			- Nhấn Add để thêm proxy.
			- ![images](./images/zb-90.png)
			
- Kiểm tra cấu hình Zabbix Proxy Group
	- 1. Kiểm tra nhóm proxy trong GUI:
		- Điều hướng đến Zabbix -> Administration -> Proxy Groups.
		- Kiểm tra xem nhóm zabbix_proxy_group đã được tạo và chứa các proxy zabbixproxy1 và zabbixproxy2 hay chưa.
		- ![images](./images/zb-91.png)
		
	- 2. Kiểm tra trạng thái proxy và phân chia host:
		- Điều hướng đến Zabbix -> Administration -> Proxies.
		- Kiểm tra cột Hosts để xem các agent đã được quản lý bởi các proxy tương ứng hay chưa. Zabbix sẽ tự động phân chia host cho các proxy trong nhóm.
		- ![images](./images/zb-92.png)

<!-- TOC --><a name="1033-kim-tra-tng-th-h-thng"></a>
#### 10.3.3 Kiểm tra tổng thể hệ thống
- Sau khi hoàn tất quá trình cài đặt và cấu hình, việc kiểm tra tổng thể hệ thống là cần thiết để xác nhận tính hoạt động và khả năng chịu lỗi của các thành phần HA.
<!-- TOC --><a name="10331-kim-tra-ha-server-cluster"></a>
##### 10.3.3.1 Kiểm tra HA Server Cluster
- Để kiểm tra cụm Zabbix Server HA, thực hiện các bước sau :
- 1. Kiểm tra trạng thái HA trên giao diện: Truy cập giao diện Zabbix Web Frontend. Điều hướng đến Reports -> System information. Phần hiển thị thông tin HA server sẽ cho biết trạng thái của các node trong cụm (Active/Standby).
	- ![images](./images/zb-93.png)
- 2. Kiểm tra kịch bản chuyển đổi dự phòng:
    - Trên node Zabbix Server đang ở trạng thái active (ví dụ: 192.168.50.140), dừng dịch vụ Zabbix Server:  
        ```Bash  
        systemctl stop zabbix-server  
		```
    - Quan sát giao diện Zabbix Web Frontend. Giao diện vẫn phải hoạt động bình thường, và node Zabbix Server còn lại (192.168.50.142) sẽ tự động chuyển sang trạng thái active.
	- ![images](./images/zb-94.png)
	- ![images](./images/zb-96.png)
	- Log trên node zabbix server 2
	- ![images](./images/zb-95.png)
    - Khởi động lại dịch vụ Zabbix Server trên node đã dừng (192.168.50.140):  
        ```Bash  
        systemctl start zabbix-server  
		```
    - Kiểm tra lại trạng thái trên giao diện Zabbix. Node 192.168.50.140 sẽ trở về chế độ standby. Quá trình này xác nhận khả năng tự động chuyển đổi dự phòng của cụm máy chủ Zabbix.
	- ![images](./images/zb-97.png)
	- ![images](./images/zb-98.png)
<!-- TOC --><a name="10332-kim-tra-proxy-group"></a>
##### 10.3.3.2 Kiểm tra Proxy Group
- Để kiểm tra hoạt động của Zabbix Proxy Group, thực hiện các bước sau :
- 1. Kiểm tra trạng thái nhóm proxy trên giao diện: Truy cập giao diện Zabbix Web Frontend. Điều hướng đến Administration -> Proxy Groups để xác nhận nhóm đã được tạo và các proxy đã được thêm vào. Sau đó, điều hướng đến Administration -> Proxies và kiểm tra cột Hosts để xem số lượng agent mà mỗi proxy đang quản lý. Các host sẽ được tự động phân chia giữa các proxy trong nhóm.
	- ![images](./images/zb-92.png)
- 2. Kiểm tra kịch bản chuyển đổi dự phòng của proxy:
    - Trên một node Zabbix Proxy (ví dụ: 192.168.50.150), dừng dịch vụ Zabbix Proxy:  
        ```Bash  
        systemctl stop zabbix-proxy  
		```
	- ![images](./images/zb-99.png)
    - Quan sát trong Zabbix GUI (tại Administration -> Proxies). Các agent trước đây báo cáo về proxy bị dừng sẽ tự động chuyển sang báo cáo về proxy còn lại (192.168.50.151). Điều này đảm bảo việc thu thập dữ liệu không bị gián đoạn.
    - Khởi động lại dịch vụ Zabbix Proxy trên node đã dừng (192.168.50.150):  
        ```Bash  
        systemctl start zabbix-proxy  
		```
    - Quan sát trong Zabbix GUI. Zabbix Server sẽ tự động cân bằng lại các host giữa hai proxy nếu có đủ số lượng host. 
	- Trong trường hợp không đủ host để chia đều, một proxy có thể ở chế độ active nhưng không quản lý host nào.
	- ![images](./images/zb-100.png)
- Việc triển khai cụm máy chủ Zabbix HA kết hợp nhóm proxy, được hỗ trợ bởi MariaDB Galera Cluster và MaxScale, tạo ra một giải pháp giám sát mạnh mẽ, có khả năng chịu lỗi cao và dễ dàng mở rộng. Kiến trúc đa lớp này đảm bảo tính liên tục của dịch vụ giám sát ngay cả khi một hoặc nhiều thành phần gặp sự cố.
- MariaDB Galera Cluster cung cấp một cơ sở dữ liệu backend đa master đồng bộ, đảm bảo tính nhất quán và sẵn sàng cao của dữ liệu giám sát. MaxScale đóng vai trò thiết yếu trong việc cân bằng tải và định tuyến truy vấn đến cụm cơ sở dữ liệu, tối ưu hóa hiệu suất và cung cấp một điểm truy cập duy nhất cho Zabbix Server và Web Frontend. Khả năng kết nối trực tiếp của Web Frontend với MaxScale cho phép truy vấn dữ liệu lịch sử ngay cả khi Zabbix Server không hoạt động hoàn toàn, nâng cao khả năng khắc phục sự cố.
- Cụm Zabbix Server HA, với cơ chế active/passive native của Zabbix 7.4, đảm bảo rằng dịch vụ giám sát cốt lõi luôn khả dụng. Đồng thời, Zabbix Proxy Group với khả năng tự động chia tải và chuyển đổi dự phòng cho phép thu thập dữ liệu phân tán một cách hiệu quả và đáng tin cậy. Việc mỗi proxy duy trì cơ sở dữ liệu cục bộ riêng cũng góp phần vào tính bền vững của dữ liệu trong các điều kiện mạng không ổn định.
