...
- [Database Server](#database-server)
   ...
   * [7. Tích Hệ và Kết Nối](#7-tích-h-và-kt-ni)
      ...
      + [7.2 ODBC/JDBC và các chuẩn kết nối](#72-odbcjdbc-và-các-chun-kt-ni)
         - [7.2.1 ODBC (Open Database Connectivity) hay kết nối cơ sở dữ liệu mở](#721-odbc-open-database-connectivity-hay-kt-ni-c-s-d-liu-m)
   ...
   * [10. Practice](#10-practice)
	  + [10.1 Thực hành cơ bản với các DMBS ](#101-thc-hành-c-bn-vi-các-dmbs)
	  + [10.2 Các lab HA, cluster](#102-các-lab-ha-cluster)
	  + [10.3 Các Lab bổ sung HA kết hợp cluster ](#103-các-lab-b-sung-ha-kt-hp-cluster)
   * [References](#references)
<!-- TOC end -->

<!-- TOC --><a name="database-server"></a>
# Database Server
<!-- TOC --><a name="1-kin-trúc-c-bn"></a>
## 1. Kiến Trúc Cơ Bản  
<!-- TOC --><a name="11-mô-hình-client-server-trong-h-thng-database"></a>
...
<!-- TOC --><a name="12-các-thành-phn-c-bn-ca-database-server"></a>
...
<!-- TOC --><a name="13-s-khác-bit-gia-database-và-database-server"></a>
...
<!-- TOC --><a name="131-database-c-s-d-liu"></a>
...
<!-- TOC --><a name="132-database-server-máy-ch-c-s-d-liu"></a>
...
<!-- TOC --><a name="14-các-loi-database-server-ph-bin-rdbms-nosql-newsql"></a>
### 1.4 Các loại database server phổ biến (RDBMS, NoSQL, NewSQL)
<!-- TOC --><a name="141-relational-database-management-system-rdbms"></a>
...
<!-- TOC --><a name="144-sql-no-sql-new-sql"></a>
...
<!-- TOC --><a name="2-h-qun-tr-c-s-d-liu-dbms"></a>
## 2 Hệ Quản Trị Cơ Sở Dữ Liệu (DBMS)
<!-- TOC --><a name="21-các-h-qun-tr-ph-bin"></a>
### 2.1 Các hệ quản trị phổ biến
<!-- TOC --><a name="211-mysqlmariadb"></a>
#### 2.1.1 MySQL/MariaDB
- MySQL 
...
<!-- TOC --><a name="212-microsoft-sql-server"></a>
...
<!-- TOC --><a name="213-oracle-database"></a>
...
<!-- TOC --><a name="215-mongodb-nosql"></a>
#### 2.1.5 MongoDB (NoSQL)
...
<!-- TOC --><a name="22-so-sánh-u-nhc-im-tng-h-thng"></a>
...
<!-- TOC --><a name="23-tiêu-chí-la-chn-dbms-phù-hp"></a>
...
<!-- TOC --><a name="3-cài-t-và-cu-hình"></a>
...
<!-- TOC --><a name="31-mysqlmariadb"></a>
### 3.1 MySQL/MariaDB
<!-- TOC --><a name="311-yêu-cu-h-thng"></a>
...
<!-- TOC --><a name="312-quy-trình-cài-t-c-bn"></a>
...
<!-- TOC --><a name="313-cu-hình-ti-u-hiu-nng"></a>
...
- MariaDB 
	...
	- File cấu hình MariaDB nằm tại 
		...
		C:\Program Files\MariaDB 11.7\data\my.ini 
		```
<!-- TOC --><a name="314-cu-hình-bo-mt-c-bn"></a>
...
<!-- TOC --><a name="32-microsoft-sql-server"></a>
### 3.2 Microsoft SQL Server
<!-- TOC --><a name="321-yêu-cu-h-thng"></a>
...
<!-- TOC --><a name="33-mongod"></a>
### 3.3 Mongod 
<!-- TOC --><a name="331-yêu-cu-h-thng"></a>
...
<!-- TOC --><a name="332-cài-t-c-bn"></a>
#### 3.3.2  Cài đặt cơ bản 
<!-- TOC --><a name="windows-1"></a>
##### Windows   
...
<!-- TOC --><a name="333-cu-hình-ti-u-hiu-nng"></a>
...
<!-- TOC --><a name="334-cu-hình-bo-mt-c-bn"></a>
...
<!-- TOC --><a name="34-postgressql"></a>
### 3.4 PostgresSQL
<!-- TOC --><a name="341-yêu-cu-h-thng"></a>
...
<!-- TOC --><a name="342-cài-t-c-bn"></a>
#### 3.4.2 Cài đặt cơ bản 
<!-- TOC --><a name="windows-2"></a>
##### Windows 
...
<!-- TOC --><a name="344-cu-hình-bo-mt-c-bn"></a>
...
- Cấu hình ghi log trong PostgreSQL  
	...
	- Khởi động lại PostgreSQL để áp dụng cấu hình:  
	  ...
	  sudo systemctl restart postgresql
	  ```
<!-- TOC --><a name="35-redis"></a>
### 3.5 Redis 
<!-- TOC --><a name="351-yêu-cu-h-thng"></a>
...
<!-- TOC --><a name="353-cu-hình-ti-u-hiu-nng"></a>
...
- Tối ưu kích thước hàng đợi kết nối (`tcp-backlog`)  
	...
	sudo systemctl restart redis
	```
<!-- TOC --><a name="354-cu-hình-bo-mt-c-bn"></a>
...
<!-- TOC --><a name="44-ti-u-hóa-truy-vn-query-optimization"></a>
...
<!-- TOC --><a name="45-qun-lý-transaction-và-lock"></a>
...
<!-- TOC --><a name="5-bo-mt-database-server"></a>
## 5. Bảo Mật Database Server
<!-- TOC --><a name="51-các-mi-e-da-bo-mt-ph-bin"></a>
...
<!-- TOC --><a name="52-mã-hóa-d-liu-encryption"></a>
...
- Transparent Data Encryption(TDE)
	...
	('Nguyen', 'Tran', 'IT', '2020-01-15', 75000.00),
	('Linh', 'Pham', 'HR', '2019-07-08', 62000.00),
	('Quang', 'Le', 'Marketing', '2021-11-01', 58000.00),
	('Thao', 'Nguyen', 'Finance', '2018-04-22', 69000.00),
	('Khoa', 'Dang', 'IT', '2022-03-13', 72000.00);
	GO
	...
	TO FILE = 'C:\temp\TDE_Cert'
	WITH PRIVATE KEY (file='C:\temp\TDE_CertKey.pvk',
	ENCRYPTION BY PASSWORD='Qaz@123') 
	```
	- ![images](./images/d-808.png)
	- ![images](./images/d-809.png)
	
	- Kiểm tra 
		- Kiểm tra list chứng chỉ
		```
		USE master;
		GO
		...
		- encryption_state = 3 nghĩa là đã được mã hóa hoàn toàn
			- Các trạng thái khác:
			- 0 = No encryption
			- 1 = Unencrypted
		...
		- Tắt Encrypt và kiểm tra trạng thái
		```
		ALTER DATABASE TestDB
		SET ENCRYPTION OFF;
		GO
		...
		- Thực hiện backup và restore DB lên 1 server khác nhưng không import cert 
		- ![images](./images/d-814.png)
		- ![images](./images/d-815.png)
		--> Báo lỗi không thể restore 
		- Thực hiện restore cert: Trên Server mới thực hiện tạo khoá chính, restore cert 
		```
		USE Master;
		GO
		CREATE MASTER KEY ENCRYPTION
		...
		GO
		CREATE CERTIFICATE TDECert
		FROM FILE = 'C:\TestDB_Cert\TDE_Cert'
		WITH PRIVATE KEY (FILE = 'C:\TestDB_Cert\TDE_CertKey.pvk',
		DECRYPTION BY PASSWORD = 'Qaz@!@3' );
		```
		- ![images](./images/d-816.png)
		- ![images](./images/d-817.png)
		- Sau khi đã restore thực hiện import DB thành công 
...
<!-- TOC --><a name="53-audit-và-theo-dõi-truy-cp"></a>
...
<!-- TOC --><a name="531-audit-trong-database"></a>
...
<!-- TOC --><a name="532-theo-dõi-truy-cp-trong-database-mc-ích-và-phng-pháp"></a>
...
<!-- TOC --><a name="533-cu-hình-vi-các-dbms-ph-bin"></a>
...
<!-- TOC --><a name="54-patch-management-và-cp-nht-bo-mt"></a>
...
<!-- TOC --><a name="6-high-availability-và-scalability"></a>
...
<!-- TOC --><a name="61-replication-ng-b-hóa"></a>
...
<!-- TOC --><a name="611-mysql-replication"></a>
...
<!-- TOC --><a name="612-sql-server-replica"></a>
...
- Logical Replication 
	...
	- Cấu hình Logical Replication 
		...
		- Hai máy đã cài PostgreSQL 14 đã khởi tạo DB 
...
<!-- TOC --><a name="62-cluster-và-failover"></a>
...
<!-- TOC --><a name="63-sharding-phân-mnh-d-liu"></a>
...
<!-- TOC --><a name="64-load-balancing-cho-database"></a>
...
<!-- TOC --><a name="7-tích-h-và-kt-ni"></a>
## 7. Tích Hệ và Kết Nối
<!-- TOC --><a name="71-các-giao-thc-kt-ni-database"></a>
...
<!-- TOC --><a name="72-odbcjdbc-và-các-chun-kt-ni"></a>
### 7.2 ODBC/JDBC và các chuẩn kết nối
<!-- TOC --><a name="721-odbc-open-database-connectivity-hay-kt-ni-c-s-d-liu-m"></a>
...
<!-- TOC --><a name="722-jdbc-java-database-connectivity"></a>
...
<!-- TOC --><a name="73-api-và-dch-v-web-cho-database"></a>
...
<!-- TOC --><a name="74-etl-extract-transform-load"></a>
...
<!-- TOC --><a name="8-giám-sát-và-khc-phc-s-c"></a>
## 8. Giám Sát và Khắc Phục Sự Cố
<!-- TOC --><a name="81-các-công-c-giám-sát-database"></a>
...
<!-- TOC --><a name="82-nhn-din-và-x-lý-bottleneck"></a>
...
<!-- TOC --><a name="83-x-lý-deadlock-và-các-vn-ng-thi"></a>
...
<!-- TOC --><a name="84-log-và-phân-tích-li"></a>
...
<!-- TOC --><a name="9-xu-hng-và-công-ngh-mi"></a>
## 9. Xu Hướng và Công Nghệ Mới
<!-- TOC --><a name="91-database-as-a-service-dbaas"></a>
...
<!-- TOC --><a name="92-in-memory-database"></a>
...
<!-- TOC --><a name="93-blockchain-database"></a>
...
<!-- TOC --><a name="94-ai-và-machine-learning-trong-qun-tr-database"></a>
...
<!-- TOC --><a name="10-practice"></a>
## 10. Practice 
<!-- TOC --><a name="101-thc-hành-c-bn-vi-các-dmbs"></a>
### 10.1 Thực hành cơ bản với các DMBS 
- Các bài thực hành với DBMS: Mysql/MariaDB, MSSQL, MongoDB, PostgreSQL, Redis
	- Các bài thực hành về database server: từ cài đặt, cấu hình, sử dụng (backup/restore, import/export), tunning được trình bày cụ thể với từng DBMS trong các phần 
		* [3. Cài Đặt và Cấu Hình](#3-cài-t-và-cu-hình)
		* [4. Quản Trị Database Server](#4-qun-tr-database-server)
		* [5. Bảo Mật Database Server](#5-bo-mt-database-server)
<!-- TOC --><a name="102-các-lab-ha-cluster"></a>
### 10.2 Các lab HA, cluster
	+ [6.1 Replication (đồng bộ hóa) ](#61-replication-ng-b-hóa)
	   - [6.1.1 MYSQL Replication](#611-mysql-replication)
	   - [6.1.2 SQL Server Replica](#612-sql-server-replica)
	   - [6.1.3 Mongod ](#613-mongod)
...
<!-- TOC --><a name="103-các-lab-b-sung-ha-kt-hp-cluster"></a>
### 10.3 Các Lab bổ sung HA kết hợp cluster 
- Bổ sung thêm các Lab thực hành HA + Cluster 
...
- Postgres HA + Cluster theo mô hình Patroni 
	- Patroni là một công cụ mã nguồn mở giúp xây dựng hệ thống High Availability (HA) cho PostgreSQL. Nó hỗ trợ tự động chuyển đổi (failover), bầu chọn leader và quản lý cấu hình. Patroni được viết bằng Python và sử dụng etcd hoặc Consul làm hệ thống đồng thuận phân tán. Công cụ này hỗ trợ nhiều phiên bản PostgreSQL và có thể triển khai trên cả môi trường on-premises và cloud.
	- Các tính năng nổi bật của Patroni
		- Tự động chuyển đổi (Automatic Failover): Khi node chính gặp sự cố, Patroni sẽ tự động đẩy một node phụ lên làm node chính mới.
		- Bầu chọn Leader (Leader Election): Thông qua etcd hoặc Consul, Patroni xác định node nào giữ vai trò primary khi hệ thống thay đổi.
		- Quản lý cấu hình: Patroni tự động đồng bộ cấu hình PostgreSQL giữa các node để đảm bảo tính nhất quán.
		- Kiểm tra sức khỏe (Health Checks): Chỉ sử dụng những node "khỏe mạnh" để nhân bản dữ liệu.
		- Tùy biến cao: Hỗ trợ các script tự định nghĩa trong các giai đoạn chuyển đổi (trước/sau promote, demote, failover).
		- Mở rộng linh hoạt: Cho phép thêm hoặc loại bỏ node phụ mà không gián đoạn dịch vụ.
	- Kiến trúc Patroni gồm 3 thành phần chính
		- Patroni Agent: Chạy trên từng máy chủ PostgreSQL, quản lý trạng thái node (promote/demote/failover).
		- etcd hoặc Consul: Quản lý đồng thuận, cấu hình và bầu chọn leader.
		- HAProxy hoặc PgBouncer: Cân bằng tải kết nối client, hỗ trợ chia tải truy vấn đọc và pooling kết nối.
		- HAProxy hoặc PgBouncer: Cân bằng tải kết nối client, hỗ trợ chia tải truy vấn đọc và pooling kết nối.
	- Triển khai PostgresSQL Clustering HA Keepalived 
	- Bảng phân bổ IP 

	| **Dịch vụ**    | **Node**   | **Địa chỉ IP** | **Vai trò**               |
	| -------------- | ---------- | -------------- | ------------------------- |
	| **VIP**        | VIP        | 192.168.50.200 | Địa chỉ ảo (Keepalived)   |
	| **HAProxy**    | haproxy-01 | 192.168.50.150 | Load balancer chính       |
	| **HAProxy**    | haproxy-02 | 192.168.50.151 | Load balancer dự phòng    |
	| **PostgreSQL** | node-01    | 192.168.50.128 | Patroni + etcd + postgres |
	| **PostgreSQL** | node-02    | 192.168.50.129 | Patroni + etcd + postgres |
	| **PostgreSQL** | node-03    | 192.168.50.130 | Patroni + etcd + postgres |

	- Mô Hình 
	```mermaid
	flowchart TD

	Client["Client/User"] --> VIP["VIP 192.168.50.200 (Keepalived)"]

	VIP --> HAProxy01["haproxy-01 192.168.50.150 (HAProxy - Chính)"]
	VIP --> HAProxy02["haproxy-02 192.168.50.151 (HAProxy - Dự phòng)"]

	HAProxy01 --> PG01["postgres-01 192.168.50.128 Patroni + etcd + PostgreSQL"]
	HAProxy01 --> PG02["postgres-02 192.168.50.129 Patroni + etcd + PostgreSQL"]
	HAProxy01 --> PG03["postgres-03 192.168.50.130 Patroni + etcd + PostgreSQL"]
	...
	- Cài đặt 
		...
		apt update && apt upgrade -y 
		```
		- Trên các node postgres 1.2.3
			- Cài đặt postgres
				- Cài đặt và update repo của postgresql 
				```
				sudo apt update
				sudo apt install -y postgresql-common
				```
				- ![images](./images/d-747.png)
				- Cài postgres 
				```
				sudo apt install -y postgresql postgresql-contrib acl
				```
				- ![images](./images/d-748.png)
				- Dừng dịch vụ
				```
				sudo systemctl stop postgresql
				sudo systemctl disable postgresql
				```
				- ![images](./images/d-749.png)
			- Cài đặt etcd 
				- Tải xuống file cài đặt 
				```
				wget https://github.com/etcd-io/etcd/releases/download/v3.5.17/etcd-v3.5.17-linux-amd64.tar.gz
				```
				- ![images](./images/d-750.png)
				- Giải nén 
				```
				tar xvf etcd-v3.5.17-linux-amd64.tar.gz
				mv etcd-v3.5.17-linux-amd64 etcd
				```
				- Di chuyển file tới thư mục khởi chạy, kiểm tra 
				```
				sudo mv etcd/etcd* /usr/local/bin/
				etcd --version
				```
				- ![images](./images/d-751.png)

			- Cài đặt patroni 
			```
			sudo apt install -y patroni
			```
			- ![images](./images/d-752.png)
		...
		- Trên các node HA 
			...
			```
			- ![images](./images/d-787.png)
			- Cài đặt keepalived
			``` 
			sudo apt install keepalived -y
			```
			- ![images](./images/d-788.png)

	- Cấu hình trên các node PostgresSQL
		- Cấu hình etcd 
			- Sinh chứng chỉ cho etcd phục vụ giao tiếp giữa etcd và postgres 
			```
			mkdir certs_etcd
			cd certs_etcd
			```
			...
			```
			- ![images](./images/d-756.png)
			- Sinh chứng chỉ cho mỗi node, chú ý alt name với từng node 
			- Node1 (192.168.50.128)
			- Sinh khoá bí mật 
			```
			...
			- Tạo file request 
			```
			openssl req -new -key etcd-node1.key -out etcd-node1.csr -subj "/C=VN/ST=HN/L=HN/O=/OU=/CN=etcd-node1" -config temp.cnf
			```
			- Ký chứng chỉ
			```
			openssl x509 -req -in etcd-node1.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out etcd-node1.crt -days 7300 -sha256 -extensions v3_req -extfile temp.cnf
			```
			- Xác minh lại chứng chỉ bằng thông số Subject Name Alternative
			```
			...
			rm temp.cnf
			```
			- ![images](./images/d-757.png)
			- Thực hiện tương tự với các node 2(192.168.50.129), node 3(192.168.50.130)
			- Node 2
			```
			...
			openssl req -new -key etcd-node2.key -out etcd-node2.csr -subj "/C=VN/ST=HN/L=HN/O=/OU=/CN=etcd-node2" -config temp.cnf
			openssl x509 -req -in etcd-node2.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out etcd-node2.crt -days 7300 -sha256 -extensions v3_req -extfile temp.cnf
			openssl x509 -in etcd-node2.crt -text -noout | grep -A1 "Subject Alternative Name"
			rm temp.cnf
			```
			- ![images](./images/d-758.png)
			- Node 3
			```
			...
			openssl req -new -key etcd-node3.key -out etcd-node3.csr -subj "/C=VN/ST=HN/L=HN/O=/OU=/CN=etcd-node3" -config temp.cnf
			openssl x509 -req -in etcd-node3.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out etcd-node3.crt -days 7300 -sha256 -extensions v3_req -extfile temp.cnf
			openssl x509 -in etcd-node3.crt -text -noout | grep -A1 "Subject Alternative Name"
			rm temp.cnf
			```
			- ![images](./images/d-759.png)

			- Sau khi thực hiện thu được 1 danh sách các chứng chỉ 
			```
			ls
			```
			- Sử dụng `scp` đẩy cert to các node 
			```
			scp ca.crt etcd-node1.crt etcd-node1.key root@192.168.50.128:/tmp/
			scp ca.crt etcd-node2.crt etcd-node2.key root@192.168.50.129:/tmp/
			scp ca.crt etcd-node3.crt etcd-node3.key root@192.168.50.130:/tmp/
			```
			- ![images](./images/d-760.png)
		- Sao chép chứng chỉ và cấp quyền: Trên từng node thực hiện 
			- Tạo user etcd service 
			```
			sudo useradd --system --home /var/lib/etcd --shell /bin/false etcd
			```
			- Tạo các thư mục cần thiết, sao chép chứng chỉ, cấp quyền 
			``` 
			...
			```
			- ![images](./images/d-761.png) 
			- Thực hiện tương tự với node 2,3 

		- Chỉnh sửa file cấu hình của ETCD tại `/etc/etcd/etcd.env`
			```
			sudo nano /etc/etcd/etcd.env
			```
			- File cấu hình với từng node sẽ có sự khác biệt cụ thể như sau:
			- Node 1
			```
			ETCD_NAME="postgresql-01"
			ETCD_DATA_DIR="/var/lib/etcd"
			ETCD_INITIAL_CLUSTER="postgresql-01=https://192.168.50.128:2380,postgresql-02=https://192.168.50.129:2380,postgresql-03=https://192.168.50.130:2380"
			...
			ETCD_INITIAL_CLUSTER_STATE="new"
			ETCD_INITIAL_CLUSTER_TOKEN="etcd-cluster"
			ETCD_INITIAL_ADVERTISE_PEER_URLS="https://192.168.50.129:2380"
			ETCD_LISTEN_PEER_URLS="https://0.0.0.0:2380"
			ETCD_LISTEN_CLIENT_URLS="https://0.0.0.0:2379"
			ETCD_ADVERTISE_CLIENT_URLS="https://192.168.50.129:2379"
			ETCD_CLIENT_CERT_AUTH="true"
			ETCD_TRUSTED_CA_FILE="/etc/etcd/ssl/ca.crt"
			...
			ETCD_INITIAL_CLUSTER_STATE="new"
			ETCD_INITIAL_CLUSTER_TOKEN="etcd-cluster"
			ETCD_INITIAL_ADVERTISE_PEER_URLS="https://192.168.50.130:2380"
			ETCD_LISTEN_PEER_URLS="https://0.0.0.0:2380"
			ETCD_LISTEN_CLIENT_URLS="https://0.0.0.0:2379"
			ETCD_ADVERTISE_CLIENT_URLS="https://192.168.50.130:2379"
			ETCD_CLIENT_CERT_AUTH="true"
			ETCD_TRUSTED_CA_FILE="/etc/etcd/ssl/ca.crt"
			...
			```
			- ![images](./images/d-764.png) 
		
			- ETCD_NAME: Tên của node trong cluster (postgresql-01, postgresql-02, postgresql-03).
			- ETCD_DATA_DIR: Thư mục lưu trữ dữ liệu etcd.
			- ETCD_INITIAL_CLUSTER: Danh sách các node etcd để hình thành cluster.
			- ETCD_INITIAL_CLUSTER_STATE: "new" khi tạo cluster mới, "existing" nếu đã có cluster trước đó.
			- ETCD_INITIAL_CLUSTER_TOKEN: Token để nhận diện cluster, tránh xung đột.
			- ETCD_INITIAL_ADVERTISE_PEER_URLS: URL mà node quảng bá để giao tiếp với các node khác.
			- ETCD_LISTEN_PEER_URLS: Cổng 2380, nơi các node etcd kết nối với nhau.
			- ETCD_LISTEN_CLIENT_URLS: Cổng 2379, nơi ứng dụng như Patroni gửi truy vấn đến etcd.
			- ETCD_ADVERTISE_CLIENT_URLS: URL để các ứng dụng biết địa chỉ truy cập etcd.
			- ETCD_CLIENT_CERT_AUTH: Bật xác thực SSL để bảo mật kết nối.
			- ETCD_TRUSTED_CA_FILE: File CA dùng để xác thực chứng chỉ SSL của etcd.
			- ETCD_CERT_FILE & ETCD_KEY_FILE: Chứng chỉ và khóa riêng của node để mã hóa dữ liệu.
			- ETCD_PEER_CLIENT_CERT_AUTH: Bật xác thực SSL giữa các node etcd.
			- ETCD_PEER_CERT_FILE & ETCD_PEER_KEY_FILE: Chứng chỉ và khóa riêng dùng cho giao tiếp giữa các node.
		...
		```
		- ![images](./images/d-768.png) 

		- Cấu hình PostgresSQL
			- Sinh chứng chỉ, gán quyền 
			```
			openssl genrsa -out server.key 2048 # private key
			openssl req -new -key server.key -out server.req # csr
			openssl req -x509 -key server.key -in server.req -out server.crt -days 7300 -copy_extensions copy # generate cert, valid for 20 years
			chmod 600 server.key
			```
			- ![images](./images/D-753.png) 
			- Sử dụng `scp` đẩy cert to các node 
			```
			scp server.crt server.key server.req root@192.168.50.128:/tmp
			scp server.crt server.key server.req root@192.168.50.129:/tmp
			scp server.crt server.key server.req root@192.168.50.130:/tmp
			```
			- ![images](./images/d-754.png) 
			- Tạo các thư mục và di chuyển, cấp quyền chứng chỉ
			```
			...
			```
			- ![images](./images/d-755.png)

			- Cấp quyền đọc cho người dùng postgres đối với các chứng chỉ etcd bằng cách sử dụng ACLs (Access Control Lists). Trên từng node chạy lệnh sau 
			```
			#Node1
		...
		- Cấu hình Patroni  
			...
			```
			- ![images](./images/d-770.png)
			- Cấu hình với từng node 
			```
			...
			etcd3:
			  hosts: 192.168.50.128:2379,192.168.50.129:2379,192.168.50.130:2379  
			  protocol: https
			...
			restapi:
			  listen: 0.0.0.0:8008
			  connect_address: 192.168.50.128:8008
			...
			bootstrap:
			  dcs:
				...
				postgresql:
					...
					- hostssl replication replicator 192.168.50.128/32 md5
					- hostssl replication replicator 192.168.50.129/32 md5
					- hostssl replication replicator 192.168.50.130/32 md5
			...
			postgresql:
			  listen: 0.0.0.0:5432
			  connect_address: 192.168.50.128:5432  
			...
			restapi:
			  listen: 0.0.0.0:8008
			  connect_address: 192.168.50.129:8008  
			  certfile: /var/lib/postgresql/ssl/server.pem
			...
			postgresql:
			  listen: 0.0.0.0:5432
			  connect_address: 192.168.50.129:5432  
			  data_dir: /var/lib/postgresql/data
			  bin_dir: /usr/lib/postgresql/14/bin
			  authentication:
				superuser:
			...
			restapi:
			  listen: 0.0.0.0:8008
			  connect_address: 192.168.50.130:8008  
			  certfile: /var/lib/postgresql/ssl/server.pem
			...
			postgresql:
			  listen: 0.0.0.0:5432
			  connect_address: 192.168.50.130:5432  
			  data_dir: /var/lib/postgresql/data
			  bin_dir: /usr/lib/postgresql/14/bin
			  authentication:
				superuser:
			...
			tags:
			  ...
			  clonefrom: false
			```
			- ![images](./images/d-781.png)
			- Khởi động Patroni và HA Cluster 
			```
			...
			```
			- ![images](./images/d-783.png)
			- ![images](./images/d-784.png)

			- Sau khi instance đã khởi tạo thực hiện sửa cấu hình etcd thực hiện trên tất cả 3 node 
			```
			sudo nano /etc/etcd/etcd.env
			```
			- Đổi giá trị `ETCD_INITIAL_CLUSTER_STATE="new"` thành `ETCD_INITIAL_CLUSTER_STATE="existing"`
			- ![images](./images/d-782.png)

			- Kiểm tra patroni cluster 
			```
			curl -k https://192.168.50.128:8008/primary | json_reformat
			```
			- ![images](./images/d-786.png)
			- Nhận thấy node1 đang là leader các node sau là replica follow theo leader1 

	- Cầu hình trên các máy HA (HA1, HA2)
		- Cấu hình HAProxy: Chỉnh sửa file cấu hình mặc định thêm phần sau 
		```
		...
		backend postgres_backend
			...
			timeout client 30s
			timeout server 30s
			server postgresql-01 192.168.50.128:5432 port 8008 check check-ssl verify none
			server postgresql-02 192.168.50.129:5432 port 8008 check check-ssl verify none
			server postgresql-03 192.168.50.130:5432 port 8008 check check-ssl verify none
		```
		- Trong đó 
			- Frontend (postgres_frontend): Mở cổng 5432, chuyển truy vấn đến postgres_backend.
			- Backend (postgres_backend): Kiểm tra leader Patroni bằng HTTP (GET /primary). Nếu trả về 200, node đó là primary.
			- PostgreSQL servers: Kết nối đến 5432, kiểm tra API Patroni trên 8008. check-ssl verify none bỏ qua kiểm tra SSL.
			
		- ![images](./images/d-789.png)
		- Khởi động HA Proxy 
		```
		sudo systemctl restart haproxy
		sudo tail -f /var/log/haproxy.log 
		```
		- ![images](./images/d-799.png)

	- Cấu hình Keepalive: Sửa file cấu hình, cần chú ý interface mạng tương ứng của mỗi máy 
	```
	...
	```
	- ![images](./images/d-800.png)
	- ![images](./images/d-801.png)
	- Tạo script check 
	```
	...
	exit 0
	```
	- Thêm user chạy script, cấu hình quyền 
	``` 
	...
	```
	- ![images](./images/d-802.png)

	- Hoàn tất cấu hình PostgreSQL HA Cluster 

	- Bổ sung 
		- Cài đặt client pgAdmin4 Web on Ubuntu 
		```
		curl -fsS https://www.pgadmin.org/static/packages_pgadmin_org.pub | sudo gpg --dearmor -o /usr/share/keyrings/packages-pgadmin-org.gpg
		sudo sh -c 'echo "deb [signed-by=/usr/share/keyrings/packages-pgadmin-org.gpg] https://ftp.postgresql.org/pub/pgadmin/pgadmin4/apt/$(lsb_release -cs) pgadmin4 main" > /etc/apt/sources.list.d/pgadmin4.list && apt update'
		sudo apt install pgadmin4-web
		```
		- ![images](./images/d-791.png)
		- Cấu hình 
		```
		sudo /usr/pgadmin4/bin/setup-web.sh
		```
		- ![images](./images/d-790.png)
		- Đăng nhập với account được tạo tại http://192.168.100.140/pgadmin4
		- ![images](./images/d-792.png)
		- Thêm server 
		- ![images](./images/d-793.png)
		- Đặt tên 
		- ![images](./images/d-794.png)
		- Cấu hình thông số: IP VIP, Port, User và pass của `postgres`
		- ![images](./images/d-795.png)
		- Đăng nhập thành công 
		- ![images](./images/d-796.png)
		- Replication Stats 
		- ![images](./images/d-797.png)
		- Phần quản lý phiên có thể thấy kết nối đang được xử lý qua HA1 
		- ![images](./images/d-798.png)
...
73. [Getting Started with MariaDB Galera and MariaDB MaxScale on CentOS](https://mariadb.com/resources/blog/getting-started-with-mariadb-galera-and-mariadb-maxscale-on-centos/)
74. [MaxScale Basic Management Using MaxCtrl for MariaDB Cluster](https://severalnines.com/blog/maxscale-basic-management-using-maxctrl-mariadb-cluster/)
75. [MaxScale configuration synchronisation](https://fromdual.com/maxscale-configurations-synchronisation)
76. [A Step-by-Step Guide to Setting Up and Configuring Galera Cluster and MaxScale Proxy](https://medium.com/anjanathenuwara/mastering-high-availability-a-complete-guide-to-configuring-galera-cluster-and-maxscale-proxy-b55ebe0749ba)