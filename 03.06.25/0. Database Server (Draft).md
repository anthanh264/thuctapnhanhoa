...
- Sau khi kích hoạt tùy chọn này, Database Engine sẽ tự động ép buộc kế hoạch được đề xuất nếu:
	...
	- Ngoài ra, hệ thống sẽ kiểm tra lại để đảm bảo kế hoạch ép buộc thực sự tốt hơn kế hoạch hiện tại.
...
MongoDB does not explicitly create a database until you insert data into it. To switch to (or create) a new database, use:
use sampleDB

This command sets the current database to sampleDB. If it doesn’t exist, MongoDB will create it when data is added.
3. Create a Collection
Collections in MongoDB are similar to tables in relational databases. You can create a collection using:
db.createCollection("users")

4. Insert Sample Data
Now, insert some sample documents into the users collection:
db.users.insertMany([
...
-  create the database user by pasting these lines and hitting ENTER on the keyboard.
```
db.createUser(
  {
    user: "Admin_Annt",
...
- Khi Redis đạt đến giới hạn bộ nhớ, nó sẽ tự động loại bỏ các key dựa trên chính sách thu hồi (eviction policy) được cấu hình thông qua thuộc tính maxmemory-policy. Dưới đây là các chính sách thu hồi và cách lựa chọn tối ưu:
1. noeviction (Không thu hồi)
...
3. volatile-lru (LRU chỉ cho key có TTL)
...
4. allkeys-random (Loại bỏ ngẫu nhiên toàn bộ key)
...
5. volatile-random (Loại bỏ ngẫu nhiên key có TTL)
...
6. volatile-ttl (Loại bỏ key có TTL thấp nhất trước)
...
7. volatile-lfu (Loại bỏ key ít được sử dụng nhất có TTL)
...
- Vô hiệu hóa các lệnh nguy hiểm trong Redis có thể vô hiệu hóa hoàn toàn các lệnh dễ bị khai thác bằng cách đặt chúng thành chuỗi rỗng trong file redis.conf.
