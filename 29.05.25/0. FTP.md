...
#### 11.3.1 Trên Windows
- FileZilla Server đã mặc định cấu hình. 
...
- Chỉnh sửa cấu hình proftpd 
	...
	- Chỉnh sửa file cấu hình sftp 
		...
		- `SFTPHostKey /etc/ssh/ssh_host_dsa_key` – Sử dụng khóa máy chủ DSA để xác thực SFTP (ít phổ biến hơn RSA).
	...
	systemctl restart proftpd
	```
- Disable SFTP Access on the SSH Port: Chỉnh sửa file cấu hình 
...
27. [Windows Server 2019 : FTP : Server World](https://www.server-world.info/en/note?os=Windows_Server_2019&p=ftp)