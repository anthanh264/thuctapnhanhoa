...
- [Web Server](#web-server)
   ...
   * [9. Thực hành/Tutorials](#9-thc-hànhtutorials)
	  ...
	  + [9.8 Monitoring Nginx ](#98-monitoring-nginx)
...
- FastCGI
	...
	- Những ưu điểm của FastCGI bao gồm:
		...
		* Đơn giản, dễ dàng chuyển đổi từ CGI: Thư viện ứng dụng FastCGI giúp đơn giản hóa việc chuyển đổi các ứng dụng CGI hiện có. Các ứng dụng được xây dựng bằng thư viện ứng dụng cũng có thể chạy như các chương trình CGI, để tương thích ngược với các máy chủ Web cũ.
...
- Prefork và Worker trong Apache 
	- Trong quá trình xử lý Multitasking thường có hai kỹ thuật chính đó là dựa vào process và thread, hay còn gọi là multiprocessing và multithreading.
		* Multiprocessing : tạo ra nhiều process và cùng xử lý một cách riêng lẻ.
...
- Event-driven 
	...
	- Kiến trúc hướng sự kiện bao gồm các nhà sản xuất sự kiện tạo ra luồng sự kiện, các đơn vị tiêu thụ sự kiện lắng nghe các sự kiện này và các kênh sự kiện chuyển sự kiện từ nhà sản xuất đến đơn vị tiêu thụ.ên lý phát hiện và phản hồi các event. Một event có thể là bất cứ điều gì xảy ra trong hệ thống, chẳng hạn như người dùng nhấp chuột, một gói tin đến từ mạng, hoặc một thay đổi trạng thái trong cơ sở dữ liệu. 
...
- mod_ssl
	...
	- Có thể tự sinh chứng chỉ SSL tự ký bằng OpenSSL và sử dụng module này để cấu hình cho Apache (Test)
		...
		* Thêm các cấu hình SSL 
		...
		```
- mod_security (Chi tiết hơn về mod_security tại [11.3.2  ModSecurity cho tường lửa ứng dụng web.](#1132-modsecurity-cho-tng-la-ng-dng-web))
...
- Ví dụ cấu hình Nginx làm proxy cho apache2 
	...
	systemctl restart nginx 
	```
- Ví dụ : [9.4  Cấu hình nginx làm reverse proxy cho apache ](#94-cu-hình-nginx-làm-reverse-proxy-cho-apache)
...
- Ví dụ sử dụng Nginx load balancing cho các backend sv1.example.com sv2.example.com sv3.example.com 
	...
	```
- Ví dụ : [9.5  Cấu hình nginx load balancing cho apache ](#95-cu-hình-nginx-load-balancing-cho-apache)
...
- Ví dụ cấu hình với Nginx 
	- Để cấu hình caching cần tạo thư mục cache trên máy chủ và đảm bảo nginx có quyền ghi vào thư mục này 
		```
		sudo mkdir -p /var/caching/nginx
		sudo chown -R nginx:nginx /var/caching/nginx
		```
	- Chỉnh sửa cấu hình trong virtual host `/etc/nginx/sites-enabled/default` sử dụng cache : 
		- Thêm vào phía trên cùng file (trước server block):
			` proxy_cache_path /var/caching/nginx levels=1:2 keys_zone=my_cache:10m max_size=100m inactive=60m use_temp_path=off;`
		- Trong đó:
			- /var/caching/nginx	Đường dẫn đến thư mục nơi cache được lưu trữ trên đĩa.
			- levels=1:2	Cấu trúc thư mục con để lưu file cache. Ở đây, Nginx sẽ tạo 2 cấp thư mục con từ hash key (ví dụ: /a/bc/). Giúp phân phối file đều và giảm tải I/O.
			- keys_zone=my_cache:10m	Tạo vùng bộ nhớ chia sẻ tên my_cache có kích thước 10MB để lưu metadata cache (như mapping key đến file).
			- max_size=100m	Tổng dung lượng tối đa của cache trên đĩa là 100MB. Cache cũ nhất sẽ bị xóa nếu vượt quá.
			- inactive=60m	Nếu một cache không được truy cập trong 60 phút, nó sẽ bị coi là “không hoạt động” và có thể bị xóa.
			- use_temp_path=off	Tắt việc sử dụng thư mục tạm để ghi cache. Cache được ghi trực tiếp vào thư mục chính – hiệu quả hơn trong hầu hết trường hợp.
	- Cấu hình chỉ định cho Nginx sử dụng cache cho các truy vấn thêm trong phần location của file VirtualHost
		```
		...
		```
		- Trong đó:
			- proxy_cache: Kích hoạt cache
			- proxy_cache_valid: Xác định thời gian lưu cache
			- proxy_cache_bypass: Cho phép client bypass cache bằng header
			- add_header: Theo dõi trạng thái cache dễ debug/tracing `X-Cache-Status: HIT|MISS|BYPASS|EXPIRED|STALE`
				| Trạng thái | Ý nghĩa                                                  |
				| ---------- | -------------------------------------------------------- |
				| `HIT`      | Nginx **đã trả lời từ cache**.                           |
				| `MISS`     | **Không có cache**, đã truy vấn backend.                 |
				| `BYPASS`   | Có `proxy_cache_bypass`, hoặc `Cache-Control: no-cache`. |
				| `EXPIRED`  | Cache đã hết hạn, truy vấn lại backend.                  |
				| `STALE`    | Trả về cache cũ tạm thời khi backend gặp lỗi.            |

	- Khởi động lại dịch vụ
	```
	systemctl restart nginx
	```
- Ví dụ: [9.6 Cấu hình caching cho Nginx ](#96-cu-hình-caching-cho-nginx)
...
- Ví dụ: [9.7 Enable HTTP/2 trên Nginx](#97-enable-http2-trên-nginx)
...
<!-- TOC --><a name="94-cu-hình-nginx-làm-reverse-proxy-cho-apache"></a>
### 9.4  Cấu hình nginx làm reverse proxy cho apache 
- Mô hình 
...
<!-- TOC --><a name="95-cu-hình-nginx-load-balancing-cho-apache"></a>
### 9.5  Cấu hình nginx load balancing cho apache 
- Mô hình 
	- 1 ubuntu nginx `192.168.100.50` 
...
<!-- TOC --><a name="96-cu-hình-caching-cho-nginx"></a>
### 9.6 Cấu hình caching cho Nginx 
- Triển khai trên máy chủ Ubuntu đã cài nginx 
```
sudo apt install nginx -y 
```
- Tạo thư mục chứa cache 
```
sudo mkdir -p /var/caching/nginx 
sudo chown -R www-data:www-data /var/caching/nginx 
```
- ![images](./images/a-113.png)

- Cấu hình caching virtual host 
Chỉnh sửa file virtual host: ví dụ ở đây sử dụng default 
```
nano /etc/nginx/sites-enabled/default
```
Thêm dòng sau vào trước khối server 
`proxy_cache_path /var/caching/nginx levels=1:2 keys_zone=my_cache:10m max_size=100m inactive=60m use_temp_path=off;`
Trong khối location /, thêm nội dung: 
```
    proxy_cache my_cache;
    proxy_cache_valid 200 302 10m;
    proxy_cache_valid 404 1m;
...
- Test 
```
curl -I http://192.168.100.137
```
- Nhận thấy có header cache từ sau lần truy cập thứ 2 
- ![images](./images/a-115.png)
- Kiểm tra thư mục cache : Có file cache được tạo 
```
tree /var/caching/nginx
```
- ![images](./images/a-116.png)
<!-- TOC --><a name="97-enable-http2-trên-nginx"></a>
### 9.7 Enable HTTP/2 trên Nginx 
- Thực hiện trên máy chủ Nginx đã enable SSL với Let’s Encrypt trong phần 
- [9.3.1 Nginx ](#931-nginx)
- Edit file virtual host thêm cấu hình http/2 `http2` và dòng `listen` trong block `server`
```
nano /etc/nginx/sites-enabled/annth.io.vn 
```
...
```
- Kiểm tra 
	- Curl chỉ lấy header để kiểm tra 
		```
		curl -I https://annth.io.vn
		```
		- Trước khi enable HTTP/2 
		- ![images](./images/a-117.png)
		- Sau khi enable HTTP/2 
...
<!-- TOC --><a name="98-monitoring-nginx"></a>
### 9.8 Monitoring Nginx 
- Monitor Nginx với prometheus và grafana 
- Mô hình 2 máy Ubuntu 
	- 1 webserver nginx có cài nginx exporter 
	- 1 server cài prometheus+grafana 
	
- Cài đặt & cấu hình 
- Trên webserver nginx 
	- Cấu hình enable module stub mặc định của Nginx 
		- Chỉnh sửa file vhost. Ở đây sử dụng file mặc định `/etc/nginx/sites-enabled/default`
		- Thêm phần sau vào trong block `server`
		```
		nano /etc/nginx/sites-enabled/default
		```
		```
		...
		nginx -s reload
		```
		![images](./images/mn-2.png)
		- Kiểm tra hoạt động 
		```
		curl http://192.168.100.137/nginx/status
		```
		![images](./images/mn-3.png)
	- Cài đặt Nginx Exporter 
		- Exporter chạy trên docker. Nếu chưa có cài đặt bằng các lệnh sau \
		```
		...
		sudo apt-get update
		sudo apt-get install gnupg docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin -y
		```
		![images](./images/mn-5.png)
		- Khởi chạy comtainer docker 
		```
		docker run\
		 -d -p 9113:9113\
		 -e SSL_VERIFY=false\
		 nginx/nginx-prometheus-exporter\
		 -nginx.scrape-uri=http://192.168.100.137/nginx/status
		```
		![images](./images/mn-4.png)
		- Kiểm tra hoạt động 
		```
		curl http://192.168.100.137:9113/metrics
		```
		![images](./images/mn-6.png)
- Trên máy Prometheus, Grafana: Cài đặt theo hướng dẫn tại [đây](https://github.com/anthanh264/thuctapnhanhoa/blob/main/08.05.25/1.%20Tri%E1%BB%83n%20khai%20Prometheus%20v%C3%A0%20Grafana.md) 
	- Cấu hình thêm target trong Prometheus
		- Chỉnh sửa file cấu hình và thêm job mới: 
		```
		...
		```
		  - job_name: "Nginx"
			...
			static_configs:
			- targets: ['192.168.100.137:9113']
		```
		- ![images](./images/mn-7.png)
		- Restart Prometheus để apply 
		```
		systemctl restart prometheus
		```
		- Kiểm tra : truy cập `http://192.168.100.132:9090/targets` 
		- ![images](./images/mn-8.png)
	- Cấu hình dashboard Grafana
		- Truy cập `http://192.168.100.132:3000` 
		- Tại `Dashboards` chọn `New Dashboards`
		- ![images](./images/mn-9.png)
		- Chọn Import
		- ![images](./images/mn-10.png)
		- Paste đoạn json dashboard trong file [này](nginx_dashboard.json) vào mục cấu hình và chọn Load
		- ![images](./images/mn-11.png)
		- Chọn source Prometheus và ấn Import 
		- ![images](./images/mn-12.png)
		- Dashboard monitor Nginx 
		- ![images](./images/mn-13.png)
...
- Open VAS
	...
	- Cài đặt Ubuntu sử dụng docker 
		- Thực hiện trên Ubuntu Server đã cài đặt Docker. TH chưa cài có thể cài thông qua các lệnh sau:
		```
		# Add Docker's official GPG key:
		sudo apt-get update
		sudo apt-get install ca-certificates curl
		...
		- Chỉnh sửa file Docker compose để có thể truy cập OpenVas remote 
			```
			127.0.0.1:9392:80
			```
			- Thành 
		...
		- OpenVAS trên Docker đang chạy  
		- ![image](./images/a-122.png)
		- ![image](./images/a-123.png)
		- Truy cập, sử dụng OpenVAS
		http://192.168.100.50:9392.
		- ![image](./images/a-124.png) 
		- Đăng nhập admin/admin 
	- Tạo target, task, kiểm tra report sau khi quét.
		- Tạo targets	
			- Chọn `Targets` trong phần `Configuration` 
			- ![image](./images/va-1.png) 
			- Click vào icon sau để tạo Target mới
			- ![image](./images/va-2.png) 
			- Cấu hình `Name` và `Host` và ấn Save 
			- ![image](./images/va-3.png) 
			- Target mới đã được tạo 
			- ![image](./images/va-4.png) 
		- Tạo task 
			- Chọn menu `Tasks` trong phần `Scan`, Chọn `New task` ở panel bên phải để tạo task mới 
			- ![image](./images/va-5.png) 
			- Cấu hình `Name` và `Scan Targets` 
			- ![image](./images/va-6.png) 
			- Cấu hình chế độ `Scan`
			- ![image](./images/va-7.png) 
			- Task mới được tạo. 
			- ![image](./images/va-8.png)
		- Khởi động task 
			- Click icon trong hình để khởi động task
			- ![image](./images/va-9.png)
			- Task khởi động thành công 
			- ![image](./images/va-10.png)
			- Task đang chạy  
			- ![image](./images/va-11.png)
			- Task hoàn thành  
			- ![image](./images/va-12.png)
		- Kiểm tra report 
			- Tại phần `Scan` chọn `Results`
			- ![image](./images/va-15.png)
			- Tại phần `Scan` chọn `Vulnerabilities`
			- ![image](./images/va-16.png)
...
* 86. [How to Secure Apache 2 With ModSecurity |* Linode Docs](https://www.linode.com/docs/guides/securing-apache2-with-modsecurity/)
