...
- FTP IIS (Windows Server):
	- Chọn sites ftp cần cấu hinh -> FTP SSL Settings  → Chọn chứng chỉ số và bật Require SSL connections.
...
- FileZilla:
  `Edit > Settings > IP Filter` → Chặn hoặc cho phép dải IP cụ thể.
...
- FTP IIS:
  Vào FTP Site > FTP IPv4 Address and Domain Restrictions → Thêm Allow/Deny IP.
...
- Tất cả hệ thống cần bắt buộc mật khẩu dài, có số, chữ hoa, ký tự đặc biệt.

### Vô hiệu hóa anonymous access:

- FileZilla:
  Không tạo user "anonymous", hoặc xóa nếu đã có.
...
## ▪️ Cập nhật phần mềm thường xuyên để vá lỗ hổng:
...
### Gợi ý thêm:
- Dùng `fail2ban` trên Linux để tự động chặn IP brute force.
- Kết hợp log và cảnh báo để theo dõi hoạt động FTP.
