...
- system() source
cho phép bạn thu thập các thông báo nhật ký cục bộ dành riêng cho hệ thống của máy chủ. Tất nhiên, điều này chỉ hoạt động trên các hệ điều hành và môi trường được các nhà phát triển và cộng tác viên syslog-ng sử dụng. .Nó cung cấp khả năng chuyển đổi dễ dàng từ /dev/log sang systemd-journal (do đó không cần phải chỉnh sửa cấu hình).

Nguồn system() tự động phân tích một số thông báo nhật ký đến (chẳng hạn như nhật ký sudo).
...
https://wiki.nhanhoa.com/kb/tim-hieu-ve-log-syslog-rsyslog-tren-linux/
https://en.wikipedia.org/wiki/Rsyslog
https://syslog-ng.github.io/admin-guide/010_Introduction_to_syslog-ng/000_What_syslog-ng_is