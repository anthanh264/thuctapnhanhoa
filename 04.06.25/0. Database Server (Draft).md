...
```
CREATE USER 'annt'@'localhost' IDENTIFIED BY 'Qaz@123';
- ![images](./images/d-177.png)  
Nếu bạn cần một người dùng chỉ có quyền truy cập vào cơ sở dữ liệu từ một địa chỉ IP cụ thể, hãy sử dụng lệnh sau:
```
...
FLUSH PRIVILEGES;
```
GRANT ALL PRIVILEGES ON test_database.* TO 'annt'@'localhost';
...
```
GRANT SELECT, INSERT, UPDATE on test_database_2.* TO 'annt'@'localhost' WITH GRANT OPTION;
- ![images](./images/d-178.png)  
- Kiểm tra 
...
REVOKE ALL PRIVILEGES ON *.* FROM 'annt'@'localhost';
- ![images](./images/d-192.png)  

- Xoá user
DROP USER 'username'@'localhost';
DROP USER 'annt'@'localhost';
...
- SQL Server có 2 loại tài khoản User là Login User và Database User.Với mỗi tài khoản User này có các vài trò và quyền các khác nhau như quyền Server Role và quyền Database Role.
một login có quyền, người dùng có thể đăng nhập vào SQL Server. Nhưng SQL Server vẫn phân biệt giữa login (cấp máy chủ) và user (cấp cơ sở dữ liệu) để kiểm soát quyền truy cập chặt chẽ hơn.
Vậy tại sao cần user nếu đã có login?
...
Phân quyền
Sau khi liên kết login với user, bạn có thể cấp quyền truy cập vào cơ sở dữ liệu hoặc bảng cụ thể.
Bằng giao diện ví dụ cấp account cho database test 
For assigning permission goto user annt property.
Click on the search option.
Select “All Objects belonging to the Schema.”
Select dbo 
- Chọn dbo cần cấu hình 
- Cấu hình các quyền ở bảng 
OK để apply 
- Test login mysql 
...
Lưu ý: Mật khẩu sẽ được mã hóa bằng MD5 (mức tối thiểu được khuyến nghị).
Thay thế your_new_username và your_password bằng tên người dùng và mật khẩu mong muốn.
Nếu bạn muốn cấp quyền superuser cho người dùng mới (chỉ cấp khi cần thiết), hãy sử dụng lệnh sau:
```
...
CREATE USER user1 WITH ENCRYPTED PASSWORD 'Qaz@123';
```
Xác minh Tạo Người Dùng
Để kiểm tra xem người dùng đã được tạo thành công, sử dụng câu lệnh sau:
```
...
```
GRANT ALL PRIVILEGES ON DATABASE db_test TO user1;
\l 
```
...
1. Tạo role cho nhóm phát triển (`dev_team`)
```sql
...
```
3. Thêm user vào role
```sql
GRANT dev_team TO user1, user2;
```
Bây giờ, user1 và user2 sẽ tự động có quyền SELECT, INSERT, UPDATE trên bảng `employees`.

4. Kiểm tra quyền của role
...
6. Xóa role nếu cần
Nếu bạn muốn xóa role `dev_team`, trước tiên cần thu hồi tất cả quyền:
```sql
...
- Redis 
Redis sử dụng Access Control List (ACL) để quản lý người dùng. Bạn có thể tạo, chỉnh sửa hoặc xóa user bằng các lệnh
...
- xoá ACL DELUSER my_user
- ![images](./images/d-232.png)  

Quản lý Quyền trong Redis
Redis cho phép quản lý quyền bằng cách cấp hoặc thu hồi quyền cho user:
Cấp quyền
Ví dụ: Cấp quyền GET, SET, DEL cho user:
ACL SETUSER my_user +get +set +del
ACL SETUSER user1 +get +set +del
- ![images](./images/d-234.png)  

Thu hồi quyền
Nếu muốn thu hồi quyền SET:
...
Back up MySQL Database Using the Binary Log
...
systemctl restart mysql 
```
Verify the binary log-related configuration options using the following SQL command to check the current binary log status:
```
mysql 
SHOW VARIABLES LIKE 'log_bin%';
```
- ![images](./images/d-237.png)  
- Manually copy the binary log files to another location as a backup. Alternatively, use tools like mysqlbinlog to extract the SQL statements from the binary logs and save them to a file. The syntax is:
mysqlbinlog /var/log/mysql/mysql-bin.xxxxxx > backup.sql

- ![images](./images/d-238.png)  
- Sử dujgn giao dijen với workbench 
...
pg_dump -U postgres -d user_management -f user_management
...
Redis
Xác định thư mục gốc của Redis
Trước khi tạo bản sao lưu cơ sở dữ liệu, bạn cần xác định thư mục gốc của Redis
config get dir
...
Postgres
- sử dụng các lệnh với các giá trị 
`pg_stat_statements` là một tiện ích mở rộng của PostgreSQL giúp theo dõi thống kê thực thi của các câu lệnh SQL. 
...
To enable `pg_stat_statements`, you need to modify the PostgreSQL configuration file (`postgresql.conf`). Add or update the following settings:
```
