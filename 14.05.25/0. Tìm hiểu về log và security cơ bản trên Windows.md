<!-- TOC --><a name="logs-và-security-c-bn-trên-windows"></a>
# Logs và Security cơ bản trên Windows 
***
<!-- TOC start-->

- [Logs và Security cơ bản trên Windows ](#logs-và-security-c-bn-trên-windows)
   * [1. Logs](#1-logs)
      + [1.1. Tổng quan ](#11-tng-quan)
      + [1.2. Các loại log và  event ](#12-các-loi-log-và-event)
   * [2. Task Scheduler ](#2-task-scheduler)
      + [2.1. Tổng quan](#21-tng-quan)
      + [2.2. Sử dụng Task Scheduler](#22-s-dng-task-scheduler)
   * [3. Windows Firewall (IN/OUT port)](#3-windows-firewall-inout-port)
      + [3.1.  Tổng quan](#31-tng-quan)
      + [3.2. Cấu hình và quản lý tường lửa ](#32-cu-hình-và-qun-lý-tng-la)
   * [4. EventViewer ](#4-eventviewer)
      + [4.1. Tổng quan](#41-tng-quan)
      + [4.2. Sử dụng EventViewer](#42-s-dng-eventviewer)
   * [5. Security cơ bản trên Windows ](#5-security-c-bn-trên-windows)
   * [6. Phân tích logs để phát hiện tấn công](#6-phân-tích-logs-phát-hin-tn-công)
      + [6.1. Phân tích Security Log](#61-phân-tích-security-log)
      + [6.2. Một số ví dụ cơ bản về việc phát hiện tấn công ](#62-mt-s-ví-d-c-bn-v-vic-phát-hin-tn-công)
   * [So sánh cơ bản Linux và Windows ](#so-sánh-c-bn-linux-và-windows)
   * [So sánh hệ thống log trên Linux và Windows ](#so-sánh-h-thng-log-trên-linux-và-windows)
...
<!-- TOC --><a name="12-các-loi-log-và-event"></a>
### 1.2. Các loại log và  event 
...
<!-- TOC --><a name="22-s-dng-task-scheduler"></a>
...
<!-- TOC --><a name="3-windows-firewall-inout-port"></a>
## 3. Windows Firewall (IN/OUT port)
<!-- TOC --><a name="31-tng-quan"></a>
### 3.1.  Tổng quan
...
<!-- TOC --><a name="32-cu-hình-và-qun-lý-tng-la"></a>
...
- Ví dụ 
	* Tạo Inbound Rule cho phép port `80`
- ![images](./images/fw-2.png)
- ![images](./images/fw-3.png)
- ![images](./images/fw-4.png)
- ![images](./images/fw-5.png)
- ![images](./images/fw-6.png)
	* Tạo Outbound Rule chặn `Edge` kết nối Internet 
- ![images](./images/fw-7.png)
- ![images](./images/fw-8.png)
- ![images](./images/fw-9.png)
- ![images](./images/fw-10.png)
- ![images](./images/fw-11.png)
- ![images](./images/fw-12.png)

<!-- TOC --><a name="4-eventviewer"></a>
## 4. EventViewer 
...
<!-- TOC --><a name="42-s-dng-eventviewer"></a>
...
<!-- TOC --><a name="5-security-c-bn-trên-windows"></a>
...
<!-- TOC --><a name="6-phân-tích-logs-phát-hin-tn-công"></a>
## 6. Phân tích logs để phát hiện tấn công
<!-- TOC --><a name="61-phân-tích-security-log"></a>
...
<!-- TOC --><a name="62-mt-s-ví-d-c-bn-v-vic-phát-hin-tn-công"></a>
...
<!-- TOC --><a name="so-sánh-c-bn-linux-và-windows"></a>
## So sánh cơ bản Linux và Windows 

|                      | LINUX SERVER                                               | WINDOWS SERVER                                                          |
| -------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------- |
| Kiến trúc            | Tập trung vào hạt nhân Linux                               | Dựa trên kiến trúc Windows NT                                           |
| Giá thành            | Phần mềm miễn phí, mã nguồn mở                             | Thuộc sở hữu của Microsoft, bao gồm chi phí cấp phép cho mỗi người dùng |
| Bảo mật              | An toàn hơn trước các mối đe dọa malware và an ninh mạng   | Dễ bị hack hơn và nhiễm virus hơn                                       |
| Hỗ trợ               | Cộng đồng lớn hỗ trợ có thể trả lời các câu hỏi thường gặp | Hỗ trợ khách hàng lâu dài, cùng với tài liệu từ Microsoft               |
| Phương pháp vận hành | Giao diện dòng lệnh                                        | Giao diện người dùng đồ họa                                             |
| Kinh nghiệm sử dụng  | Yêu cầu một quản trị viên Linux có kinh nghiệm tương đối   | Dễ sử dụng hơn đối với người mới bắt đầu                                |


<!-- TOC --><a name="so-sánh-h-thng-log-trên-linux-và-windows"></a>
## So sánh hệ thống log trên Linux và Windows 
|    Tính năng           |         Linux                                                                                                                                                                                           |     Windows                                                                                                                                                                                                                     |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hệ thống ghi log       | Sử dụng syslog (truyền thống) và systemd journal (hiện đại). Log được lưu trữ trong các file văn bản, thường tại /var/log/. Cấu hình thông qua /etc/rsyslog.conf hoặc cấu hình journald.                | Sử dụng Windows Event Log. Log được lưu trữ trong các file nhị phân (.evt hoặc .evtx). Được quản lý và xem thông qua Event Viewer.                                                                                              |
| Cấu trúc log           | Các file log thường là văn bản thuần túy, mỗi dòng chứa thông tin về thời gian, hostname, process, và thông điệp log. Dễ dàng xử lý bằng các công cụ dòng lệnh.                                         | Log được cấu trúc rõ ràng với các trường thông tin như Time, Source, Event ID, Task Category, Level, User, Computer. Dễ dàng lọc và xem thông qua Event Viewer.                                                                 |
| Công cụ xem log        | Các công cụ dòng lệnh mạnh mẽ như cat, grep, tail, less, journalctl cho phép xem, lọc và phân tích log trực tiếp từ terminal.                                                                           | Công cụ Event Viewer cung cấp giao diện đồ họa để xem, lọc, tìm kiếm và quản lý các sự kiện log. Có thể xuất log ra nhiều định dạng khác nhau.                                                                                  |
| Mức độ log (Severity)  | Định nghĩa các mức độ quan trọng của log (ví dụ: debug, info, notice, warning, err, crit, alert, emerg) theo chuẩn syslog.                                                                              | Sử dụng các mức độ như Information, Warning, Error, Critical, Verbose để phân loại mức độ nghiêm trọng của sự kiện.                                                                                                             |
| Quản lý quyền truy cập | Sử dụng hệ thống quyền sở hữu và quyền truy cập dựa trên user, group và others với các quyền read, write, execute (rwx). Quản lý bằng các lệnh chmod và chown.                                          | Sử dụng Access Control Lists (ACLs) cho phép gán quyền chi tiết hơn cho từng user hoặc group trên các đối tượng (files, folders, registry keys...). Quản lý thông qua giao diện đồ họa hoặc lệnh icacls.                        |
| Firewall cơ bản        | iptables (truyền thống) và firewalld (hiện đại) là các tường lửa mạnh mẽ, cấu hình thông qua dòng lệnh với nhiều tùy chọn phức tạp.                                                                     | Windows Firewall with Advanced Security cung cấp giao diện đồ họa và dòng lệnh (netsh, PowerShell) để cấu hình các quy tắc tường lửa dựa trên port, protocol, ứng dụng...                                                       |
| Kiểm soát người dùng   | Hệ thống user và group được quản lý thông qua các file như /etc/passwd, /etc/shadow, /etc/group. Các lệnh như useradd, userdel, groupadd, groupdel để quản lý.                                          | Quản lý người dùng và nhóm thông qua Local Users and Groups (lusrmgr.msc) hoặc Active Directory. Chính sách mật khẩu và các thiết lập bảo mật khác được quản lý thông qua Local Security Policy (secpol.msc) hoặc Group Policy. |
| Theo dõi bảo mật       | Việc theo dõi log (đặc biệt là /var/log/auth.log hoặc các log liên quan đến auditd) rất quan trọng để phát hiện các hoạt động đáng ngờ. Các công cụ như auditctl có thể được sử dụng để cấu hình audit. | Theo dõi Security log trong Event Viewer là cách chính để giám sát các sự kiện liên quan đến bảo mật như đăng nhập không thành công, thay đổi quyền, truy cập tài nguyên nhạy cảm.                                              |
| Cập nhật bảo mật       | Các bản vá và cập nhật bảo mật thường được quản lý thông qua các trình quản lý gói (ví dụ: apt trên Debian/Ubuntu, yum hoặc dnf trên Red Hat/CentOS/Fedora).                                            | Các bản vá và cập nhật bảo mật được quản lý thông qua Windows Update.                                                                                                                                                           |

- Linux mang đến sự linh hoạt và khả năng tùy biến cao hơn trong cả việc ghi log và cấu hình bảo mật, đặc biệt thông qua các công cụ dòng lệnh và file cấu hình. Điều này đòi hỏi người dùng có kiến thức kỹ thuật sâu hơn để tận dụng tối đa.
- Windows cung cấp giao diện đồ họa trực quan và các công cụ quản lý tích hợp, giúp người dùng dễ dàng quản lý log và cấu hình các thiết lập bảo mật cơ bản mà không cần quá nhiều kiến thức về dòng lệnh. Tuy nhiên, sự tùy biến có thể không sâu sắc bằng Linux.
<!-- TOC --><a name="references"></a>
## References
1. [Where Are the Windows Logs Stored?](https://www.liquidweb.com/blog/where-are-the-windows-logs-stored/)
2. [Windows Event Log](https://www.solarwinds.com/resources/it-glossary/windows-event-log)
3. [How to create an automated task using Task Scheduler on Windows 10](https://www.windowscentral.com/how-create-automated-task-using-task-scheduler-windows-10)
4. [Windows Firewall](https://learn.microsoft.com/en-us/windows/security/operating-system-security/network-security/windows-firewall/)
5. [What is User Account Control (UAC)?](https://kb.blackbaud.com/knowledgebase/articles/Article/75433)
6. [Windows Forensic Part3 Windows Event Logs](https://sec.vnpt.vn/2023/02/windows-forensicpart3-windows-event-logs)