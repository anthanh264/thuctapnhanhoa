...
```
EventViewer 
Event Viewer là một công cụ trong Windows hiển thị thông tin chi tiết về các sự kiện quan trọng trên máy tính của bạn. Ví dụ về các sự kiện này là các chương trình không khởi động như mong đợi hoặc tự động tải xuống các bản cập nhật. Event Viewer đặc biệt hữu ích để khắc phục sự cố Windows và lỗi ứng dụng.
- Khởi chạy 
Để khởi động Event Viewer, có thể thực hiện một số cách sau đây:
...
- Sử dụng
Trang điều hướng, theo mặc định được định vị ở bên trái, cung cấp cas tùy chọn để chọn nhật ký sự kiện để xem. Có năm danh mục trong Nhật ký Windows:
...
Bạn cũng có thể xem Audit successes và failures của Audit, liên quan đến các sự kiện bảo mật.
Các sự kiện được liệt kê theo thứ tự thời gian, bắt đầu với sự kiện mới nhất ở trên cùng. Ngoài ra, bạn có thể nhấp vào các cột để chỉnh sửa thứ tự và nhóm.
- ![images](./images/ev-4.png)
- ![images](./images/ev-5.png)
- Trang Actions 
Trang cuối cùng nằm ở bên phải theo mặc định là trang Actions, cung cấp cho bạn quyền truy cập nhanh vào các tính năng có sẵn cho bạn tại thời điểm này. Trang này được chia thành hai phần, phần đầu tiên chứa các hành động có sẵn cho trang Điều hướng đã chọn. Phần thứ hai chứa các hành động có sẵn cho chính sự kiện đã chọn.
