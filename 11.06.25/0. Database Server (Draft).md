...
- Postgres 
	...
	- Ví dụ: Tại mô hình replica physical của Postgres thực hiện sử dụng pg_ctl promote để chuyển đổi failover 
		- Mô hình physical replica ở phần trước 
...
- Sharding là phân vùng ngang dữ liệu – thay vì lưu toàn bộ dữ liệu trong một cơ sở dữ liệu duy nhất, chia nó thành nhiều phần nhỏ, mỗi phần lưu trên một máy chủ hoặc instance riêng biệt. Mỗi shard chứa một phần dữ liệu và hoạt động như một cơ sở dữ liệu độc lập.
- Dữ liệu quá lớn khiến truy vấn chậm.
- Hệ thống cần mở rộng theo chiều ngang (thêm máy chủ).
- Cần tăng khả năng chịu lỗi (mỗi shard độc lập).
- Ứng dụng có lượng người dùng hoặc giao dịch rất lớn (ví dụ: mạng xã hội, thương mại điện tử, IoT).
