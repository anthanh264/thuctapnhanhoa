...
- Ví dụ tuning max connections 
	- Apache2 : 
		- Cần đảm bảo apache có sử dụng module MPM. Ví dụ tắt Prefork enable Worker.
		```
		a2dismod mpm_prefork
		a2enmod mpm_worker
		```
		- Chỉnh sửa file cấu hinh `/etc/apache2/mods-available/mpm_worker.conf`
		```
		...
		- Cấu hình các thông số 
			...
			```
			- ServerLimit: Số lượng tối đa các tiến trình Apache. Đây là giới hạn về số child processes mà Apache có thể tạo ra.
			- StartServers: Số lượng tiến trình sẽ khởi động khi bạn bắt đầu chạy Apache.
			- MinSpareThreads / MaxSpareThreads: Số lượng luồng rảnh rỗi (idle threads) tối thiểu/tối đa được giữ lại mà không bị dừng. Giúp Apache có sẵn luồng để xử lý request mới nhanh chóng.
			- ThreadsPerChild: Số lượng luồng trên mỗi tiến trình con (child process). Mỗi tiến trình con sẽ chứa số luồng này để xử lý các yêu cầu.
			- **MaxRequestWorkers**: Số lượng kết nối đồng thời tối đa mà Apache có thể hỗ trợ. Đây là chỉ thị chính cần thay đổi để tăng giới hạn kết nối đồng thời trong Apache.
			- MaxConnectionsPerChild: Số lượng kết nối mà mỗi tiến trình con sẽ xử lý trước khi nó bị dừng và khởi động lại. Đặt 0 nghĩa là không giới hạn.
...
- Ví dụ cấu hình keep-alive 
	- Apache 
		...
		- Cấu hình các thông số Keep-Alive trong Apache như sau:
			...
			- KeepAliveTimeout: Số giây chờ cho yêu cầu tiếp theo trên cùng một kết nối. Mặc định là 5 giây. Đặt quá cao có thể gây ra nhiều kết nối nhàn rỗi, làm giảm hiệu suất máy chủ. Chỉ điều chỉnh khi người dùng gặp lỗi kết nối.
		- Reload Apache để apply 
			```
			sudo systemctl restart apache2
			```
